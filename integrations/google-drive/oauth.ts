 ...
 *
 * Shares all Google OAuth wire-format with Gmail and Calendar via
 ...
 *
 * What's Drive-specific:
 *   - Redirect URL: /api/integrations/oauth/google-drive/callback.
 *   - accountId lookup: OIDC userinfo (the `userinfo.email` scope grants it).
 *     Drive's own API doesn't expose a thin getProfile-like endpoint that
 *     works with the `drive` scope alone, so the userinfo endpoint is the
 *     cleanest source of truth for the user's email — same pattern Calendar
 *     uses.
 *
 * Env vars read:
...
function getRedirectUrl(): string {
  ...
  return `${baseUrl}/api/integrations/oauth/google-drive/callback`;
}
...
export const googleDriveOAuth: ProviderOAuth = {
  ...
  async handleCallback(code, _state, pkce) {
    if (pkce === null || !pkce.codeVerifier) {
      throw new Error(
        "googleDriveOAuth.handleCallback: PKCE code_verifier is required for Google Drive; the consumed oauth_states row had none.",
      );
    }
  ...
  async revoke(_token: string): Promise<void> {
    // Deferred to disconnect-UX slice (matches Gmail + Calendar + Slack patterns).
  },
};
