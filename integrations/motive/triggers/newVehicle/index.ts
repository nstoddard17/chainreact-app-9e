...
/**
 * Module-init registration for Motive `new_vehicle` — MOTIVE-1.
 * Motive `vehicle_upserted`; first-seen dedup (id-only key) so an update to an
 * existing vehicle never re-fires the "new" workflow. One of 7 company-webhook
 * triggers.
 */
const TYPE: MotiveTriggerType = "new_vehicle";
const activate = buildMotiveActivate(TYPE);
export const motiveNewVehicleFilter = makeMotiveCompanyFilter(TYPE);
registerActivation("motive", TYPE, activate);
registerDeactivation("motive", TYPE, motiveSharedDeactivate);
registerTriggerFilter(motiveNewVehicleFilter);
