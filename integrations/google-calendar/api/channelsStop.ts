...
/**
 * Wrapper for Google Calendar `channels.stop`.
 *
 ...
 *
 * Endpoint: POST {base}/calendar/v3/channels/stop
...
export async function channelsStop(input: ChannelsStopInput): Promise<void> {
  const url = `${calendarApiBase()}/calendar/v3/channels/stop`;
  ...
  if (res.status === 401) {
    throw new Unauthorized401Error(
      "Google Calendar channels.stop returned HTTP 401",
    );
  }
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(
      `Google Calendar channels.stop failed: ${surfaceErrorDetail(text, res.status)}`,
    );
  }
