import { z } from "zod";
import type { TriggerEvent } from "@/contracts/triggerEvent";
import type {
  FilterResult,
  TriggerFilter,
} from "@/core/triggers/filterContract";

/**
 * Filter for `asana:task_updated_in_project` — Slice 5.ASANA-1.
 *
 * Same projectId narrowing as `newTaskInProject/filter.ts` — the
 * dispatcher's (provider, eventType) fan-out is global, so this is what
 * keeps a project-B change from firing a project-A workflow.
 */
const ConfigSchema = z.object({
  projectId: z.string().min(1),
});
type Config = z.infer<typeof ConfigSchema>;

export const asanaTaskUpdatedInProjectFilter: TriggerFilter<Config> = {
  provider: "asana",
  eventType: "task_updated_in_project",
  parseConfig(rawConfig: unknown): Config {
    return ConfigSchema.parse(rawConfig);
  },
  evaluate(event: TriggerEvent, config: Config): FilterResult {
    const eventProjectGid = event.payload.projectGid;
    if (eventProjectGid === config.projectId) return { kind: "match" };
    return {
      kind: "no-match",
      reason: `project ${String(eventProjectGid)} does not match filter ${config.projectId}`,
    };
  },
};
