...
/**
 * `microsoft-powerbi:pipeline_stage_dataflows` options resolver.
 *
 * Backs `dataflowIds` on the selective-deploy action. Same multi-parent
 ...
 */
export const microsoftPowerBiPipelineStageDataflowsResolver: OptionsResolver =
  {
    source: "microsoft-powerbi:pipeline_stage_dataflows",
    ...
    async resolve(ctx) {
      ...
      try {
        ...
        throw new OptionsResolverError(
          "PROVIDER_ERROR",
          "Couldn't load Power BI stage dataflows. Try again.",
        );
      }

      const items = artifacts.dataflows.map((a) => ({
        value: a.id,
        label: a.name,
      }));
