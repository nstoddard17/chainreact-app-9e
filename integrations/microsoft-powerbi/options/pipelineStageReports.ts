...
/**
 * `microsoft-powerbi:pipeline_stage_reports` options resolver.
 *
 * Backs `reportIds` on the selective-deploy action. Same multi-parent
 ...
 */
export const microsoftPowerBiPipelineStageReportsResolver: OptionsResolver = {
  source: "microsoft-powerbi:pipeline_stage_reports",
  ...
  async resolve(ctx) {
    ...
    try {
      ...
      throw new OptionsResolverError(
        "PROVIDER_ERROR",
        "Couldn't load Power BI stage reports. Try again.",
      );
    }

    const items = artifacts.reports.map((a) => ({
      value: a.id,
      label: a.name,
    }));
