 ...
 *
 * Shares all Google OAuth wire-format with Gmail, Calendar, and Drive via
 ...
 *
 * What's Sheets-specific:
 *   - Redirect URL: /api/integrations/oauth/google-sheets/callback.
 *   - accountId lookup: OIDC userinfo (the `userinfo.email` scope grants it).
 *     Sheets' own API doesn't expose a thin getProfile-like endpoint that
 *     works with the `spreadsheets` scope alone, so the userinfo endpoint
 *     is the cleanest source of truth for the user's email — same pattern
 ...
 *
 * revoke is a stub deferred to the disconnect-UX slice (matches Gmail,
 * Calendar, Drive, and Slack patterns).
 */
...
function getRedirectUrl(): string {
  ...
  return `${baseUrl}/api/integrations/oauth/google-sheets/callback`;
}
...
export const googleSheetsOAuth: ProviderOAuth = {
  ...
  async handleCallback(code, _state, pkce) {
    if (pkce === null || !pkce.codeVerifier) {
      throw new Error(
        "googleSheetsOAuth.handleCallback: PKCE code_verifier is required for Google Sheets; the consumed oauth_states row had none.",
      );
    }
  ...
  async revoke(_token: string): Promise<void> {
    // Deferred to disconnect-UX slice (matches Gmail + Calendar + Drive + Slack patterns).
  },
};
