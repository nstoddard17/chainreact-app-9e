...
| [`chainreactv2-template-author`](./chainreactv2-template-author/SKILL.md) | The **repeatable, locked seed-batch procedure** (set by batches 1 and 2). Adding, auditing, or maintaining official templates: data-only forward-only idempotent SQL migration, `source='official'`, `account_id`/`created_by_user_id` NULL, `'ChainReact'`, `visibility='public'`, empty configs, fixed UUIDs + `ON CONFLICT (id) DO NOTHING`, validated against the live registry. Complements the builder skill above. | A `_seed_official_templates_batch_N.sql` migration + updated aggregate tests + local commit + batch report. |
