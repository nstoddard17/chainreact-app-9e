...
//
// NOT WIRED YET: the engine still uses recordRun (finalize-only INSERT) until
// COST-15C. These functions add the data-layer capability without changing any
// execution or billing behavior. All write via service-role (no user session).
//
// Type scoping: the new non-terminal 'running' state lives on this lifecycle
// type, NOT on the display-facing WorkflowRunStatus / WorkflowRunRecord above.
// Those serve getById/listByWorkflow/recordRun, which only ever read TERMINAL
// rows until the engine is wired (COST-15C); widening them would ripple into the
// API contract (WorkflowRunSummarySchema) + UI for no current benefit. COST-15C
// widens the display types when it makes running rows user-visible.
