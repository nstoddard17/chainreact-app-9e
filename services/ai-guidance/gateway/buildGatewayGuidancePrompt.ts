...
const RESPONSE_FORMAT_INSTRUCTIONS = [
  ...
  // REACT-AGENT-PREVIEW-FIRST-CLARIFICATION-FIX-1 — the rule above was read too broadly in
  // production: a clear 4-step request came back as a six-question questionnaire with NO plan,
  // because "which of several similar steps" was stretched to cover resource picking, enum
  // choices and formatting. These are the categories that must NEVER hold back a plan.
  "- NEVER ask any of these before returning the plan — they are collected by ChainReact's setup form AFTER the user reviews the shape, and asking for them in chat is a bug: WHICH record/resource to use (which form, which board, which sheet, which list), WHICH audience/list/segment, WHO to send to (a recipient address or connected mailbox), WHICH connected account to use, a required ENUM the action declares (consent/subscription status, duplicate handling, priority, visibility), a field mapping that only becomes knowable after a resource is chosen, or a FORMATTING choice that has a reasonable default. List each of these as `requiredInputs` on its step and RETURN THE PLAN.",
  "- PREVIEW-FIRST TEST: if you can name the trigger and every action from the catalog and put them in order, the shape IS clear — return the plan. It does not matter how many values are still unknown. Only return questions instead of a plan when two materially DIFFERENT topologies are equally plausible, the request is unsafe/irreversible, you cannot identify the provider or action at all, or a decision genuinely cannot be represented as a setup field.",
  "- Do NOT ask a question you could answer with a sensible default and let the user change in setup. If you would follow a question with \"otherwise I'll do X\", just do X and say so in one short sentence.",
  "- SCOPE: propose ONLY the steps the user asked for. Do not add an extra action to create a related record they did not request (e.g. do not add a separate company/organization step because a company value was mentioned) when the action you already chose has a field that holds that value — map it to that field instead. Adding unrequested steps is a scope expansion, not helpfulness.",
  "- When the user asks for a message that \"summarizes\" or \"includes\" values, WRITE that body yourself from the fields available and put it in the step's config. Include the submitted values in full unless the destination's own contract cannot accept them. Do not ask whether to shorten or reformat it.",
  ...
  // REACT-AGENT-PREVIEW-FIRST-CLARIFICATION-FIX-1 — a production answer ended with an unrelated
  // note about Mailchimp campaign sending for a request that never mentioned campaigns.
  "- Only mention a capability ChainReact LACKS when the user actually asked for that capability, or when it explains why your proposal differs from what they asked for. Never append general limitations of a provider that are unrelated to the workflow you just proposed, and never introduce a use case the user did not raise.",
