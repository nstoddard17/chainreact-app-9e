...
const RESPONSE_FORMAT_INSTRUCTIONS = [
  ...
  "- CHANGING an existing workflow (the 'Current workflow shape' above is non-empty and the user asks to change/switch/replace a step, e.g. 'change it to an email notification') is ALSO a plan, not prose. Return the UPDATED plan — the full intended workflow with the requested change applied (e.g. the same trigger, with the Slack step replaced by an email action). Keep the steps the user didn't ask to change. Do NOT reply that the change is fine without returning the updated plan, and do NOT claim you changed anything (ChainReact applies plans only after the user clicks Apply).",
