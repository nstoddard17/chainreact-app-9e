 ...
 * ANALYTICS-SOURCES-ONENOTE-1). No I/O — unit tested directly.
 *
 * The date window + bucket plan are built HERE from the dashboard range; widget config
 * never supplies a raw Graph query. Only COUNTS + page created/modified timestamps are
 * derived — never a page title, content, preview, or author.
 */

export const MAX_BUCKETS = 12;
const DAY_MS = 86_400_000;

export interface OneNoteTimeBucket {
...
export function planBuckets(
  ...
  const buckets: OneNoteTimeBucket[] = [];
...
export function bucketIndexForMs(buckets: readonly OneNoteTimeBucket[], ms: number | null): number {
  if (ms === null) return -1;
