/**
 * Pure day-bucketing helpers shared by the Google Docs + Google Sheets analytics
 * sources (Slice ANALYTICS-SOURCES-GWORKSPACE-1). No I/O — unit tested directly.
 *
 * The date window + bucket plan are built HERE from the dashboard range; widget config
 * never supplies a raw Drive query. Only COUNTS + file created/modified timestamps are
 * derived — never a file name, content, owner, or permission.
 */

export const MAX_BUCKETS = 12;
const DAY_MS = 86_400_000;

export interface WorkspaceTimeBucket {
...
export function planBuckets(
  ...
  const buckets: WorkspaceTimeBucket[] = [];
...
export function bucketIndexForMs(buckets: readonly WorkspaceTimeBucket[], ms: number | null): number {
  if (ms === null) return -1;
