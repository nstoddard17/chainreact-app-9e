...
import { OneDriveRateLimitError, scanDrive, type FileFact } from "./api";
import { bucketIndexForMs, parseFolderId, planBuckets } from "./buckets";

/**
 * Microsoft OneDrive connected-app analytics source — v1
 * (Slice ANALYTICS-SOURCES-ONEDRIVE-1). Mirrors the Dropbox file-analytics pattern on
 * Microsoft Graph.
 *
 * READ-ONLY + METADATA-ONLY. Reduces a bounded recursive `children` traversal of a
 * folder (or drive root) to numeric file aggregates (file/folder counts,
 * files-modified over time, files-by-type). Never executes a workflow node, never
 * takes a raw Graph query from widget config (the folder is a validated DriveItem id;
 * `$select`/`$top` are server-side constants), and never reads a file name, path,
 * webUrl, owner, content, preview, or sharing link. Approved metrics only.
 *
 * CREDENTIAL MODEL (personal provider, per core/integrations/credentialSharing.ts):
 * OneDrive is PERSONAL, so this resolves the REQUESTING USER'S OWN connection
 ...
 * MISSING_CREDENTIAL. OneDrive is REFRESHABLE → reads run through `refreshAndRetry`.
 *
 * PRIVACY: only COUNTS + a file's modified-bucket + extension are computed and cached.
 * No file names, paths, ids, sizes, webUrls, owners (createdBy/lastModifiedBy),
 * content, previews, sharing links, or raw Graph payloads are returned or stored.
 * (Extension — a type category like "pdf" — is the only text surfaced, on
...
const PROVIDER_KEY = "microsoft-onedrive";
const FOLDER_FILTER = ["onedrive_folder"] as const;
...
const METRICS: readonly AnalyticsSourceMetric[] = [
  {
    key: "files_count",
    label: "Files",
    description: "Files in the selected folder (or all of OneDrive).",
  ...
  },
  {
    key: "folders_count",
    label: "Folders",
    description: "Subfolders in the selected folder (or all of OneDrive).",
    visualizations: ["scalar"],
    supportedGroupBy: [],
    supportedFilters: FOLDER_FILTER,
  },
...
/** Map any error from a OneDrive read into a typed, leak-free AnalyticsSourceError. */
function classifyOneDriveError(err: unknown): AnalyticsSourceError {
  ...
  if (err instanceof IntegrationActionRequiredError || err instanceof Unauthorized401Error) {
    return new AnalyticsSourceError("Reconnect OneDrive before this widget can load.", "MISSING_CREDENTIAL");
  }
  if (err instanceof OneDriveRateLimitError) {
    return new AnalyticsSourceError("OneDrive's rate limit was reached. Try again shortly.", "RATE_LIMITED");
  }
  if (err instanceof NotFoundError) {
    return new AnalyticsSourceError(
      "ChainReact can't read that OneDrive folder. Re-pick a folder.",
      "INVALID_QUERY",
    );
  }
  // No raw Graph error body / token / path / payload leaks — a generic, safe message.
  return new AnalyticsSourceError("Couldn't load OneDrive data.", "PROVIDER_ERROR");
}

/** Resolve the requesting user's OWN OneDrive connection (or MISSING_CREDENTIAL). */
async function resolveOwnOneDrive(ctx: AnalyticsSourceContext): Promise<{ providerAccountId: string | null }> {
  const integration = await getActiveForExecution(ctx.accountId, PROVIDER_KEY, null, {
    connectedByUserId: ctx.userId,
  });
  if (!integration) {
    throw new AnalyticsSourceError("Connect OneDrive to use this widget.", "MISSING_CREDENTIAL");
  }
...
export const microsoftOneDriveAnalyticsSource: AnalyticsSourceAdapter = {
  ...
  async query(
    ...
    if (!metric) {
      throw new AnalyticsSourceError(`Unknown OneDrive metric: ${query.metricKey}`, "UNKNOWN_METRIC");
    }

    // Validate the optional folder filter server-side BEFORE any I/O ("" = root).
    let folderId: string;
    try {
      folderId = parseFolderId(query.filters?.onedrive_folder);
    } catch (err) {
      throw new AnalyticsSourceError(
        err instanceof Error ? err.message : "Pick a OneDrive folder.",
        "INVALID_QUERY",
      );
    }

    const generatedAt = new Date().toISOString();
    const { providerAccountId } = await resolveOwnOneDrive(ctx);
    ...
    try {
      ...
      throw classifyOneDriveError(err);
    }
    ...
    // ── Series: files modified over time (by lastModifiedDateTime) ────────────
