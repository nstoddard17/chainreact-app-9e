...
export const PATCH_SHAPE_GUIDE = [
  ...
  "Catalog entries are written `provider:type` (e.g. `slack:send_direct_message`). In a node you MUST split that key: set provider to the part before the colon (\"slack\") and type to the part after it (\"send_direct_message\"). Never put the combined `provider:type` string into either field. GOOD: `{ \"provider\": \"gmail\", \"type\": \"new_email\" }`. BAD: `{ \"provider\": \"gmail\", \"type\": \"gmail:new_email\" }` — the `type` must NOT repeat the provider, or the node resolves to nothing.",
