/**
 * Slice 3.42 integration test — Notion `append_block_children` config
 * end-to-end through the live WorkflowBuilder shell.
 *
 * Canonical UX-shape test for the Slice 3.42 paste-JSON surface
 ...
 *     plan §3.2),
 *   - the saved `children` value is the literal JSON string the
 *     author pastes,
 *   - Modal Save flushes the draft into pendingNodes,
 *   - Modal Save does NOT call updateWorkflow,
 *   - Toolbar Save persists once via updateWorkflow with both fields
 *     intact.
 */
...
const CHILDREN_JSON =
  '[{"type":"heading_2","text":"Notes"},{"type":"paragraph","text":"Captured by ChainReact."}]';
...
beforeEach(() => {
  ...
  mockListProviderActions.mockImplementation(async (p: string) =>
    p === "notion" ? [notionAppendBlockChildrenMeta] : [],
  );
...
it("end-to-end: type blockId + paste children JSON → Modal Save (draft only) → Toolbar Save (updateWorkflow once with literal JSON string)", async () => {
  ...
  // 2. Drill into Notion → Append Block Children.
  await user.click(screen.getByRole("button", { name: /add action/i }));
  await user.click(
    screen.getByRole("button", { name: /browse notion actions/i }),
  );
  await waitFor(() => {
    expect(screen.getByText("Append Block Children")).toBeInTheDocument();
  });
  await user.click(screen.getByText("Append Block Children"));
