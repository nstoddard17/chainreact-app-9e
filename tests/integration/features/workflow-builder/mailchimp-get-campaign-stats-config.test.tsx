/**
 * Slice 3.MAILCHIMP-4 integration test — Mailchimp `get_campaign_stats`
 * config end-to-end through the live WorkflowBuilder shell.
 *
 * Pins:
 *   - meta surface: single required `campaignId` combobox sourced
 *     from `mailchimp:campaigns`; engagement aggregates
 *     (opens/clicks/bounces/forwards) are nested sensitive object
 *     projections; `industryStats` stays structural (aggregate
 *     benchmarks, not customer data).
 *   - end-to-end: pick stats via `mailchimp:campaigns` → Modal Save →
 *     Toolbar Save persists once with `{campaignId: "..."}`.
 *
 * Out of scope: see sibling get_campaign config test.
 */
...
beforeEach(() => {
  ...
  mockListProviderActions.mockImplementation(async (p: string) =>
    p === "mailchimp" ? [mailchimpGetCampaignStatsMeta] : [],
  );
...
it("Mailchimp get_campaign_stats meta — single required campaignId combobox + low-risk + engagement aggregates marked sensitive + industryStats structural — Slice 3.MAILCHIMP-4 meta guard", () => {
  expect(mailchimpGetCampaignStatsMeta.riskLevel).toBe("low");
  expect(mailchimpGetCampaignStatsMeta.isDestructive).toBe(false);
  expect(mailchimpGetCampaignStatsMeta.requiresConfirmation).toBe(false);

  expect(mailchimpGetCampaignStatsMeta.fields.map((f) => f.name)).toEqual([
    "campaignId",
  ]);
  const f = mailchimpGetCampaignStatsMeta.fields[0]!;
  expect(f.type).toBe("combobox");
  expect(f.optionsSource).toBe("mailchimp:campaigns");
  expect(f.required).toBe(true);

  for (const name of ["opens", "clicks", "bounces", "forwards"] as const) {
    const o = mailchimpGetCampaignStatsMeta.outputs.find((x) => x.name === name)!;
    expect(o.type).toBe("object");
    expect(o.sensitive).toBe(true);
    expect((o.fields ?? []).length).toBeGreaterThan(0);
  }
  // Industry benchmarks are aggregate Mailchimp data; structural only.
  const industry = mailchimpGetCampaignStatsMeta.outputs.find(
    (o) => o.name === "industryStats",
  )!;
...
it("end-to-end: pick campaign via mailchimp:campaigns → Modal Save → Toolbar Save persists ONCE with EXACT runtime field name `campaignId`", async () => {
  ...
  await waitFor(() =>
    expect(screen.getByText("Get Campaign Stats")).toBeInTheDocument(),
  );
  await user.click(screen.getByText("Get Campaign Stats"));
  const action = useGraphSlice
    .getState()
    .pendingNodes.find((n) => n.kind === "action")!;
  expect(action.provider).toBe("mailchimp");
  expect(action.type).toBe("get_campaign_stats");
  ...
  expect(persistedAction.type).toBe("get_campaign_stats");
