...
describeDb("accounts RLS — Slice 4.ACCOUNT-MODEL-3", () => {
  ...
  async function createTestUser(label: string): Promise<{
    ...
    const email = `accounts-rls-${slug}@chainreact.test`;
    ...
    if (error || !data.user) {
      throw new Error(`createTestUser: ${error?.message ?? "no user"}`);
    }
  ...
  afterAll(async () => {
    ...
    for (const id of createdUserIds) {
      ...
      if (error) {
        console.warn(`cleanup: failed to delete user ${id}: ${error.message}`);
      }
    }
  });
