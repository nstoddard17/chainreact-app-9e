 ...
 * account-keyed after 4.ACCOUNT-MODEL-9c). The accountBilling repo wrappers are
 * mocked — we assert the service's flag / test-mode gating, result mapping,
 * error handling, and no-leak guarantees without a DB. The feature flag is
 * toggled via process.env at call time.
 */
...
describe("createBillingReservation — gating", () => {
  it("flag disabled → skipped/disabled, no RPC call, no mutation", async () => {
    const r = await createBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", estimatedTasks: 3 });
  ...
  it("test mode → skipped/test_mode, no RPC call (even when enabled)", async () => {
    enable();
    const r = await createBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", estimatedTasks: 3, testMode: true });
...
describe("createBillingReservation — enabled", () => {
  it("successful reserve maps faithfully and passes params (incl. expiry)", async () => {
    ...
    const r = await createBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", estimatedTasks: 3, expiresAt: "2026-05-25T01:00:00Z" });
    expect(mockReserve).toHaveBeenCalledWith("acct-1", 3, "run-1", "2026-05-25T01:00:00Z");
  ...
  it("zero estimate → explicit zero_reservation (RPC still called with 0, expiry null)", async () => {
    ...
    const r = await createBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", estimatedTasks: 0 });
    expect(mockReserve).toHaveBeenCalledWith("acct-1", 0, "run-1", null);
  ...
  it("idempotent re-reserve preserves already_reserved", async () => {
    ...
    const r = await createBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", estimatedTasks: 4 });
...
describe("reconcileBillingReservation", () => {
  it("flag disabled → skipped/disabled, no RPC call", async () => {
    const r = await reconcileBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", actualTasks: 2 });
  ...
  it("test mode → skipped/test_mode", async () => {
    enable();
    const r = await reconcileBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", actualTasks: 2, testMode: true });
  ...
  it("success maps charged/refunded + status reconciled", async () => {
    ...
    const r = await reconcileBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", actualTasks: 2 });
    expect(mockReconcile).toHaveBeenCalledWith("acct-1", "run-1", 2);
  ...
  it("over-reserve reason preserved", async () => {
    ...
    const r = await reconcileBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", actualTasks: 5 });
  ...
  it("not-reserved RPC reason normalizes to not_reserved", async () => {
    ...
    const r = await reconcileBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", actualTasks: 1 });
...
describe("releaseBillingReservation", () => {
  it("flag disabled → skipped/disabled, no RPC call", async () => {
    const r = await releaseBillingReservation({ accountId: "acct-1", workflowRunId: "run-1" });
    expect(r).toMatchObject({ ok: true, skipped: true, reason: "disabled" });
  ...
  it("success maps released + status released", async () => {
    ...
    const r = await releaseBillingReservation({ accountId: "acct-1", workflowRunId: "run-1" });
    expect(mockRelease).toHaveBeenCalledWith("acct-1", "run-1");
...
describe("no secret / internals leakage in service results", () => {
  it("results contain only ids/counts/enums/amounts + operational error — no secrets", async () => {
    ...
    const results = JSON.stringify([
      await createBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", estimatedTasks: 1 }),
      await reconcileBillingReservation({ accountId: "acct-1", workflowRunId: "run-1", actualTasks: 1 }),
      await releaseBillingReservation({ accountId: "acct-1", workflowRunId: "run-1" }),
