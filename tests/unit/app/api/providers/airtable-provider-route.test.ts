...
describe("GET /api/providers/airtable/actions", () => {
  ...
  it("recordId stays a typed text field (no record picker)", async () => {
    const byKey = new Map((await fetchActions()).map((a) => [a.key, a]));
    for (const key of ["airtable:get_record", "airtable:update_record", "airtable:delete_record", "airtable:add_attachment"]) {
      const recordId = byKey.get(key)!.fields.find((f) => f.name === "recordId")!;
      expect(recordId.type).toBe("text");
      expect(recordId.optionsSource).toBeUndefined();
    }
  });
  ...
  it("no action field references the rejected airtable:records resolver", async () => {
    for (const a of await fetchActions()) {
