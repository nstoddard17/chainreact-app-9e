 ...
 *
 * Source-level guard (reads the file as text) so it never imports the server-only
 * `writeHarnessDeps` module graph (enqueueRun / supabase / engine).
 */
import { readFileSync } from "node:fs";
import { resolve } from "node:path";

const SEAM_FILE = resolve(__dirname, "../../smoke-actions/writeHarnessDeps.ts");
...
describe("write-harness smoke seams use refreshAndRetry (no raw token reads)", () => {
  const source = readFileSync(SEAM_FILE, "utf8");
  const lines = source.split(/\r?\n/);

  it("never decrypts a token directly — every provider read goes through refreshAndRetry", () => {
    // refreshAndRetry decrypts internally; a `decryptToken(` in the seam file means
    // someone hand-rolled a raw provider call that bypasses the refresh path.
    const offenders = lines
      .map((l, i) => ({ l, n: i + 1 }))
      .filter(({ l }) => /\bdecryptToken\s*\(/.test(l));
    expect(offenders.map((o) => `L${o.n}: ${o.l.trim()}`)).toEqual([]);
  });

  it("imports + uses refreshAndRetry for the seam reads", () => {
    expect(source).toMatch(/import\s*\{\s*refreshAndRetry\s*\}/);
    // At least one refreshAndRetry per refreshable/handler-wrapped provider seam.
    const count = (source.match(/refreshAndRetry\s*\(/g) ?? []).length;
  ...
  it("calls each provider read wrapper only inside a refreshAndRetry apiCall lambda", () => {
    ...
    for (const wrapper of PROVIDER_READ_WRAPPERS) {
      ...
      lines.forEach((line, idx) => {
        ...
        violations.push(`L${idx + 1} (${wrapper}): ${line.trim()}`);
      });
    }
