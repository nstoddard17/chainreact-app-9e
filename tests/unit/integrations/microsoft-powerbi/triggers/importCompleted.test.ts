...
import { activate } from "@/integrations/microsoft-powerbi/triggers/importCompleted/activate";

const NOW = Date.parse("2026-07-15T12:00:00Z");
const EVENT_TYPE = "import_completed";
...
beforeEach(() => {
  ...
  mockImportGet.mockResolvedValue({
    ...
    updatedDateTime: "2026-07-15T11:32:00Z",
    reports: [],
    datasets: [],
  });
});
...
describe("import_completed activation", () => {
  it("seeds the already-Succeeded import ids and enables polling", async () => {
    mockImportsList.mockResolvedValueOnce([
      summary({ id: "imp-2" }),
      summary({ id: "imp-1" }),
      summary({ id: "imp-0", importState: "Failed" }),
    ]);

    const result = await activate(activationCtx());

    expect(mockImportsList).toHaveBeenCalledWith(
      expect.objectContaining({ groupId: "ws-1" }),
    );
  ...
  it("does NOT seed a Publishing import, so it still fires when it succeeds", async () => {
    mockImportsList.mockResolvedValueOnce([
      summary({ id: "imp-9", importState: "Publishing" }),
    ]);
...
describe("import_completed poll", () => {
  it("emits one event per newly-Succeeded import, enriched with the import's timestamps", async () => {
    mockImportsList.mockResolvedValueOnce([
      summary({ id: "imp-2" }),
      summary({ id: "imp-1" }),
    ]);
    ...
    // Get Imports In Group carries no timestamps — only the NEW import is
    // read back individually.
    expect(mockImportGet).toHaveBeenCalledTimes(1);
    expect(mockImportGet).toHaveBeenCalledWith(
      expect.objectContaining({ groupId: "ws-1", importId: "imp-2" }),
    );

    expect(mockEnqueue).toHaveBeenCalledTimes(1);
    const event = mockEnqueue.mock.calls[0]![0].event;
    expect(event.eventType).toBe(EVENT_TYPE);
    expect(event.eventId).toBe("microsoft-powerbi:wf-1:n-1:import_completed:imp-2");
  ...
  it("does not emit (or read back) imports in a non-matching state", async () => {
    mockImportsList.mockResolvedValueOnce([
      summary({ id: "imp-3", importState: "Failed" }),
