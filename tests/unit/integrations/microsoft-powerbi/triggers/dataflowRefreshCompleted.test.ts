...
import { activate } from "@/integrations/microsoft-powerbi/triggers/dataflowRefreshCompleted/activate";

const NOW = Date.parse("2026-07-15T12:00:00Z");
const EVENT_TYPE = "dataflow_refresh_completed";
...
function transaction(overrides: Record<string, unknown>) {
  return {
    id: "2026-07-15T11:40:55.09Z@abc$1",
    refreshType: "OnDemand",
    startTime: "2026-07-15T11:40:55Z",
    endTime: "2026-07-15T11:45:00Z",
    status: "Success",
    ...overrides,
  };
}
...
describe("dataflow_refresh_completed activation", () => {
  it("seeds the already-successful transaction ids and enables polling", async () => {
    mockTransactionsList.mockResolvedValueOnce({
      transactions: [
        transaction({ id: "t-2" }),
        transaction({ id: "t-1", status: "Failed" }),
      ],
      hasMore: false,
    });
    ...
    expect(mockTransactionsList).toHaveBeenCalledWith(
      expect.objectContaining({ groupId: "ws-1", dataflowId: "df-1" }),
    );
  ...
  it("does NOT seed an in-flight transaction, so it still fires when it succeeds", async () => {
    mockTransactionsList.mockResolvedValueOnce({
      transactions: [transaction({ id: "t-9", status: "InProgress" })],
      hasMore: false,
    });
...
describe("dataflow_refresh_completed poll", () => {
  it("emits one event per newly-successful transaction with the bounded payload", async () => {
    mockTransactionsList.mockResolvedValueOnce({
    ...
    expect(event.eventId).toBe(
      "microsoft-powerbi:wf-1:n-1:dataflow_refresh_completed:t-2",
    );
  ...
  it("matches the status case-insensitively (the enum is undocumented)", async () => {
    mockTransactionsList.mockResolvedValueOnce({
      transactions: [transaction({ id: "t-2", status: "success" })],
      hasMore: false,
    });
  ...
  it("does not emit for other statuses or a null status", async () => {
    mockTransactionsList.mockResolvedValueOnce({
      transactions: [
        transaction({ id: "t-3", status: "Failed" }),
        transaction({ id: "t-4", status: "InProgress" }),
        transaction({ id: "t-5", status: null }),
      ],
      hasMore: false,
    });
