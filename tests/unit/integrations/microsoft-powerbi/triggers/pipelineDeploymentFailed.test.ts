...
import { activate } from "@/integrations/microsoft-powerbi/triggers/pipelineDeploymentFailed/activate";

const NOW = Date.parse("2026-07-15T12:00:00Z");
const EVENT_TYPE = "pipeline_deployment_failed";
...
beforeEach(() => {
  ...
  mockOperationGet.mockResolvedValue({
    operationId: "op-2",
    status: "Failed",
    executionStartTime: "2026-07-15T11:00:00Z",
    executionEndTime: "2026-07-15T11:03:00Z",
    sourceStageOrder: 0,
    targetStageOrder: 1,
    errorCode: "DeploymentFailed_ArtifactConflict",
  });
});
...
describe("pipeline_deployment_failed activation", () => {
  it("seeds the already-Failed operation ids and enables polling", async () => {
    mockOperationsList.mockResolvedValueOnce([
      operation({ operationId: "op-2" }),
      operation({ operationId: "op-1", status: "Succeeded" }),
    ]);
...
describe("pipeline_deployment_failed poll", () => {
  it("emits with the error code read back from the operation detail", async () => {
    mockOperationsList.mockResolvedValueOnce([
      operation({ operationId: "op-2" }),
    ...
    // The operations LIST carries no error surface — only the NEW failure
    // is read back individually.
    expect(mockOperationGet).toHaveBeenCalledTimes(1);
    expect(mockOperationGet).toHaveBeenCalledWith(
      expect.objectContaining({ pipelineId: "pl-1", operationId: "op-2" }),
    );
    ...
    expect(event.eventId).toBe(
      "microsoft-powerbi:wf-1:n-1:pipeline_deployment_failed:op-2",
    );
  ...
  it("emits errorCode null when the operation detail reports no code", async () => {
    ...
    expect(mockEnqueue.mock.calls[0]![0].event.payload.errorCode).toBeNull();
  });

  it("does not emit (or read back) succeeded or in-flight operations", async () => {
    mockOperationsList.mockResolvedValueOnce([
      operation({ operationId: "op-3", status: "Succeeded" }),
  ...
  it("leaks no access token, provider host, or raw error-details blob into the payload", async () => {
    mockOperationsList.mockResolvedValueOnce([operation({ operationId: "op-2" })]);
    ...
    expect(payload).not.toContain("errorDetails");
    expect(payload).not.toContain("executionPlan");
    // `performedBy` carries a user principal name — never surfaced.
