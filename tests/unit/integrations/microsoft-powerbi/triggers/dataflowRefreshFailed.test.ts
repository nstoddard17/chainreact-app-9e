...
import { activate } from "@/integrations/microsoft-powerbi/triggers/dataflowRefreshFailed/activate";

const NOW = Date.parse("2026-07-15T12:00:00Z");
const EVENT_TYPE = "dataflow_refresh_failed";
...
describe("dataflow_refresh_failed activation", () => {
  it("seeds the already-failed transaction ids and enables polling", async () => {
...
describe("dataflow_refresh_failed poll", () => {
  it("emits one event per newly-failed transaction with the bounded payload", async () => {
    mockTransactionsList.mockResolvedValueOnce({
    ...
    expect(event.eventId).toBe(
      "microsoft-powerbi:wf-1:n-1:dataflow_refresh_failed:t-2",
    );
  ...
  it("does not emit for successful or in-flight transactions", async () => {
    mockTransactionsList.mockResolvedValueOnce({
      transactions: [
