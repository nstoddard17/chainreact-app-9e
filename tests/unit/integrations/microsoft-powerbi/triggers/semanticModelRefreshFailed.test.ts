...
import { activate } from "@/integrations/microsoft-powerbi/triggers/semanticModelRefreshFailed/activate";

const NOW = Date.parse("2026-07-15T12:00:00Z");
const EVENT_TYPE = "semantic_model_refresh_failed";
...
describe("semantic_model_refresh_failed activation", () => {
  it("seeds the already-Failed request ids and enables polling", async () => {
...
describe("semantic_model_refresh_failed poll", () => {
  it("emits with the error code parsed from the provider's exception JSON", async () => {
    mockRefreshesList.mockResolvedValueOnce({
      refreshes: [entry({ refreshRequestId: "req-2" })],
      hasMore: false,
    });
    ...
    expect(event.eventId).toBe(
      "microsoft-powerbi:wf-1:n-1:semantic_model_refresh_failed:req-2",
    );
  ...
  it("emits errorCode null when the provider reported no code", async () => {
    mockRefreshesList.mockResolvedValueOnce({
      refreshes: [entry({ refreshRequestId: "req-2", errorCode: null })],
      hasMore: false,
    });
  ...
  it("does not emit for pre-existing failures or for other statuses", async () => {
    mockRefreshesList.mockResolvedValueOnce({
      refreshes: [
        entry({ refreshRequestId: "req-1" }),
        entry({ refreshRequestId: "req-9", status: "Completed" }),
      ],
      hasMore: false,
    });
  ...
  it("leaks no access token or raw provider exception blob into the payload", async () => {
    mockRefreshesList.mockResolvedValueOnce({
      refreshes: [entry({ refreshRequestId: "req-2" })],
      hasMore: false,
    });
