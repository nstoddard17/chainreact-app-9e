...
jest.mock("@/integrations/google-calendar/triggers/eventChanged/pull", () => ({
...
const baseTrigger = {
  ...
  nodeId: "n1",
  config: { channelId: "channel-1", syncToken: "sync-1", calendarId: "primary" },
  accountId: null,
  registeredAt: "",
  expiresAt: null,
  lastRenewedAt: null,
...
describe("receiveCalendarWebhook", () => {
  ...
  it("returns unknown_channel when no trigger row matches the channelId", async () => {
    ...
    const result = await receiveCalendarWebhook(
      makeRequest({
  ...
  it("pulls delta and returns events on resource_state=exists", async () => {
    mockListByConfigContains.mockResolvedValueOnce([baseTrigger]);
    mockVerifyChannelToken.mockReturnValueOnce(true);
      ...
      events: [
        {
          ...
          payload: { changeKind: "created", calendarId: "primary", eventId: "evt-1" },
        },
      ],
    ...
    if (result.kind === "events") {
      ...
      expect(result.events[0]!.eventId).toBe("evt-1:t1");
    }
