...
import { activate } from "@/integrations/microsoft-excel/triggers/updatedRow/activate";
...
function ctx(): {
  ...
  const node: WorkflowNode = {
    id: "n-1",
    type: "microsoft-excel:updated_row",
    provider: "microsoft-excel",
    kind: "trigger",
...
describe("updated_row activation hook", () => {
  ...
  it("seeds an empty snapshot for an empty worksheet (single null cell envelope)", async () => {
    mockUsed.mockResolvedValueOnce({
      address: "Sheet1!A1",
      rowCount: 1,
      columnCount: 1,
  ...
  it("propagates Graph errors so activation fails closed (closes V1 first-poll-miss bug)", async () => {
    ...
    await expect(activate(ctx())).rejects.toThrow(/timeout/);
  });
  ...
  it("rejects missing worksheetName in node.config (Zod fail)", async () => {
    const c = ctx();
    (c.node.config as { worksheetName?: string }).worksheetName = "";
    await expect(activate(c)).rejects.toThrow();
  });
});
