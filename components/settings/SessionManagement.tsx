...
export function SessionManagement({ className }: SessionManagementProps) {
  ...
  const revokeSession = async (sessionId: string) => {
    ...
    try {
      ...
      toast({ title: "Error", description: "Failed to revoke session. Please try again.", variant: "destructive" })
    } finally {
      setRevoking(null)
    }
  }
  ...
  const revokeAllSessions = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "All Sessions Revoked", description: "You have been signed out from all devices." })
      } else {
        throw new Error("Failed to revoke sessions")
      }
    } catch (error) {
      toast({ title: "Error", description: "Failed to revoke sessions. Please try again.", variant: "destructive" })
    }
  }
  ...
  // Combine current session + other sessions into one list, current first
  const allSessions = [
    ...(currentSession ? [currentSession] : []),
    ...sessions.filter((s) => !s.is_current && !s.revoked_at),
  ]

  return (
    <div className={cn("space-y-4", className)}>
      {/* Header with Sign Out All */}
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {allSessions.length} active {allSessions.length === 1 ? 'session' : 'sessions'}
        </p>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={revokeAllSessions}>Sign Out All</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        )}
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  session.is_current && "border-primary/20"
                )}
              >
