...
/**
 * Admin-only floating debug panel for troubleshooting
 ...
 * - Export capabilities
 */
export function AdminDebugPanel({ title, data, className = "" }: AdminDebugPanelProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed bottom-4 right-4 z-50 bg-white dark:bg-slate-900 border rounded-lg shadow-2xl flex flex-col"
          style={{
          ...
          {!isMinimized && (
            <>
              <div className="text-xs text-muted-foreground px-3 pt-2 pb-1">
                Admin-only • Captures events in real-time as you navigate
              </div>
                ...
                <TabsContent value="logs" className="flex-1 flex flex-col overflow-hidden mt-2">
                  <div className="flex justify-between items-center mb-2">
                    <p className="text-xs text-muted-foreground">
                      Navigate the app to see live events
                    </p>
                  ...
                  <div className="flex-1 overflow-auto border rounded-lg bg-slate-950 p-3 font-mono text-xs">
                    {events.length === 0 ? (
                      <div className="text-slate-400 text-center py-8 text-xs">
                        No events captured yet. Navigate to /apps or other pages to see logs.
                      </div>
                    ) : (
              ...
              <div className="border-t p-2 text-[10px] text-gray-500">
                Admin-only panel • Regular users don't see this
              </div>
            </>
          )}
        </div>
