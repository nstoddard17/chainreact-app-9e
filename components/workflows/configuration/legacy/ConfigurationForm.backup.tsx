...
import { getAirtableFieldType, getAirtableFieldTypeFromSchema, isAIField, getAIFieldName, createAIFieldPlaceholder, isEditableFieldType } from './utils/airtableHelpers';
import { filterRecordsbySearch, inferFieldType, formatFieldValue, getDiscordBotInviteUrl } from './utils/helpers';
import { DiscordProgressiveConfig } from './components/discord/DiscordProgressiveConfig';
import { FieldsWithTable } from './components/FieldsWithTable';
import { useDiscordState } from './hooks/useDiscordState';
import { useFileFieldHandler } from './hooks/useFileFieldHandler';
...
/* Helper function to map Airtable field types from schema to form field types - MOVED TO utils/airtableHelpers.ts
...
export default function ConfigurationForm({
  ...
  // Discord state moved to useDiscordState hook
  ...
  // Discord state moved to useDiscordState hook
  // const [channelBotStatus, setChannelBotStatus] = useState<{ isInChannel: boolean; canSendMessages: boolean; hasPermissions: boolean; userCanInviteBot: boolean; error?: string } | null>(null);
  ...
  // Image preview is now handled by useFileFieldHandler hook
  ...
  // File field handler hook
  const fileFieldHandler = useFileFieldHandler({
    fieldSuggestions,
    setFieldSuggestions,
    activeBubbles,
    setActiveBubbles,
  ...
  // Extract imagePreview from fileFieldHandler
  const { imagePreview, setImagePreview } = fileFieldHandler;

  // Airtable bubble handler hook
  ...
  // Provider field handlers hook
  const providerHandlers = useProviderFieldHandlers({
    nodeInfo,
    values,
    setValue,
    loadOptions,
  ...
  // Discord bot status checking moved to useDiscordState hook
  /* const checkBotStatus = useCallback(async (guildId: string) => {
    if (!guildId || !discordState?.discordIntegration) return;
  ...
  }, [discordState?.discordIntegration, loadOptions]);

  // Discord reactions loading is now handled by useDiscordState hook
  // const loadReactionsForMessage - moved to discordState
  ...
  }, [values, nodeInfo?.providerId, loadOptions]);

  // Discord channel bot status checking is now handled by useDiscordState hook
  // const checkChannelBotStatus - moved to discordState

  // Discord bot server checking moved to useDiscordState hook
  // const checkBotInServer - moved to discordState
  ...
  /* const handleInviteBot = useCallback((guildId?: string) => {
    ...
    let inviteUrl = `https://discord.com/api/oauth2/authorize?client_id=${discordState.discordClientId}&permissions=${permissions}&scope=bot%20applications.commands`;
  ...
  }, [discordClientId, isDiscordBotConfigured, discordState.checkBotInServer, loadOptions]); */
    ...
    const fetchDiscordConfig = async () => {
      try {
        ...
        if (data.configured && data.clientId) {
          ...
          discordState.setDiscordClientId(data.clientId);
        } else {
  ...
  useEffect(() => {
    ...
    if (nodeInfo.providerId === 'discord' && !discordState.botStatus && hasInitialized) {
      discordState.setBotStatus({
        isInGuild: true,
        hasPermissions: true
      });
    }
  ...
  const renderDiscordProgressiveConfig = () => {
    return (
      <DiscordProgressiveConfig
        nodeInfo={nodeInfo}
        values={values}
        errors={errors}
        discordIntegration={discordState.discordIntegration}
        botStatus={discordState.botStatus}
        isBotStatusChecking={discordState.isBotStatusChecking}
        isBotConnectionInProgress={discordState.isBotConnectionInProgress}
        loadingFields={loadingFields}
        dynamicOptions={dynamicOptions}
        handleFieldChange={handleFieldChange}
        handleConnectDiscord={discordState.handleConnectDiscord}
        handleInviteBot={discordState.handleInviteBot}
        handleAddBotToServer={discordState.handleAddBotToServer}
        handleDynamicLoad={handleDynamicLoad}
  ...
  /* OLD Discord progressive config implementation - replaced with DiscordProgressiveConfig component
    ...
    if (!discordState?.discordIntegration) {
      return (
    ...
    if (!channelField && values.guildId && discordState.botStatus?.isInGuild && discordState.botStatus?.hasPermissions) {
    ...
    if (values.guildId && (!discordState.botStatus || discordState.isBotStatusChecking)) {
    ...
    if (!channelField && values.guildId && discordState.botStatus && !discordState.botStatus.isInGuild) {
      return (
        <ScrollArea className="h-[calc(90vh-180px)] pr-4">
          <div className="space-y-6 p-4">
            <FieldRenderer
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
              ...
              <Button
                ...
                onClick={() => discordState.handleAddBotToServer(values.guildId)}
                disabled={discordState.isBotConnectionInProgress}
              >
                {discordState.isBotConnectionInProgress ? (
                  <div className="flex items-center gap-2">
                    <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></div>
                    Adding Bot...
                  </div>
          ...
          <div className="space-y-6 p-4">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
              ...
              <Button
                ...
                onClick={() => discordState.handleInviteBot(values.guildId)}
                disabled={discordState.isBotConnectionInProgress}
              >
                {discordState.isBotConnectionInProgress ? (
                  <div className="flex items-center gap-2">
    ...
    if (values.guildId && discordState.botStatus?.isInGuild && !discordState.botStatus.hasPermissions) {
      return (
        <ScrollArea className="h-[calc(90vh-180px)] pr-4">
          <div className="space-y-6 p-4">
            <FieldRenderer
    ...
    if (values.guildId && discordState.botStatus?.isInGuild && discordState.botStatus?.hasPermissions && channelField && !values.channelId) {
  ...
  }, [getIntegrationByProvider, airtableTableSchema]);
  ...
  }, [getIntegrationByProvider, values, airtableTableSchema]);
  ...
  const handleFieldChange = useCallback(async (fieldName: string, value: any, skipBubbleCreation: boolean = false) => {
    ...
    // Check if this is a file field (now handled by the hook)
    const airtableFieldType = fieldName.startsWith('airtable_field_') && airtableTableSchema?.fields
      ? airtableTableSchema.fields.find((f: any) => f.id === fieldName.replace('airtable_field_', ''))?.type
      : null;
    ...
    // OLD bubble creation logic - commented out as it's now handled by the hook
    /* if (!skipBubbleCreation && isUpdateRecord && fieldName.startsWith('airtable_field_') && value && value !== '' && !fileFieldHandler.isFileField(field, airtableFieldType)) {
    ...
    } */// End of UPDATE RECORD bubble logic
    
    // CREATE RECORD bubble logic is now handled by the useAirtableBubbleHandler hook
    ...
    if (!skipBubbleCreation && isCreateRecord && fieldName.startsWith('airtable_field_') && value && value !== '' && !fileFieldHandler.isFileField(field, airtableFieldType)) {
      // Get field info from table schema
      const fieldId = fieldName.replace('airtable_field_', '');
      const tableField = airtableTableSchema?.fields?.find((f: any) => f.id === fieldId);
    ...
    } */ // End of CREATE RECORD bubble logic
    ...
    // File field handling has been moved to the useFileFieldHandler hook above
    ...
    if (fieldName === 'tableName' && nodeInfo?.providerId === 'airtable') {
      ...
      if (nodeInfo.type === 'airtable_action_update_record' || 
        ...
        if (nodeInfo.type === 'airtable_action_update_record' && value && values.baseId) {
          if (typeof loadAirtableRecords === 'function') {
    ...
    } */ // End of provider-specific field handling (Discord, Google Sheets, Airtable)
    
    // Don't auto-save configuration changes - let user save manually when they click Save or Listen
  }, [nodeInfo, setValue, loadOptions, values, discordState?.discordIntegration, discordState?.checkBotStatus, discordState?.checkChannelBotStatus, discordState?.checkBotInServer, currentNodeId, getWorkflowId, dynamicOptions, setShowPreviewData, setPreviewData, setSelectedRecord, setAirtableRecords, setLoadingRecords, isLoadingInitialConfig, activeBubbles, fieldSuggestions, originalBubbleValues]);
    ...
    if (nodeInfo.providerId === 'discord' && nodeInfo.type?.includes('discord_action')) {
      ...
      const fields = visibleFields.filter(field => {
        ...
        if (field.name === 'channelId') {
          ...
          const hasDiscordConnected = !!discordState?.discordIntegration;
  ...
  /**
   * Render fields for the configuration form
   */
  const renderFields = useCallback((fields: any[], isDynamic: boolean = false) => {
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (showPreviewData) {
                          ...
                          if (typeof loadGoogleSheetsPreviewData === 'function') {
                  ...
                  <div className="border-t border-slate-200 bg-slate-50 px-3 py-2 rounded-b-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          // Refresh the records
                          if (values.baseId && values.tableName && typeof loadAirtableRecords === 'function') {
                            setLoadingRecords(true);
  ...
  console.log('🔍 [ConfigForm] Provider check:', {
    ...
    isDiscordAction: nodeInfo?.type?.startsWith('discord_action_'),
    willUseDiscordUI: nodeInfo?.providerId === 'discord' && nodeInfo?.type?.startsWith('discord_action_')
  });
  ...
  return (
    <form onSubmit={(e) => {
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {advancedFields.length > 0 ? (
          ...
          <div className="flex-1 min-h-0">
            <TabsContent value="basic" className="h-full mt-0">
              <ScrollArea className="h-[calc(90vh-220px)] pr-4">
                <div className="space-y-3 px-2 pb-6">
                  {typeof handleDynamicLoad === 'function' && typeof renderFields === 'function' && renderFields(basicFields, false)}
                  ...
                  {dynamicFields.length > 0 && handleDynamicLoad && renderFields(dynamicFields, true)}
            ...
            <TabsContent value="advanced" className="h-full mt-0">
              <ScrollArea className="h-[calc(90vh-220px)] pr-4">
                <div className="space-y-3 px-2 pb-6">
                  {handleDynamicLoad && renderFields(advancedFields, false)}
                  {/* Render dynamic fields for advanced tab */}
                  {dynamicAdvancedFields.length > 0 && handleDynamicLoad && renderFields(dynamicAdvancedFields, true)}
                </div>
              </ScrollArea>
            </TabsContent>
          </div>
        ...
        <ScrollArea className="h-[calc(90vh-180px)] pr-4">
          <div className="space-y-3 px-2 pb-6">
            {handleDynamicLoad && renderFields(basicFields, false)}
      ...
      {nodeInfo?.providerId === 'discord' && (
        <div className="px-6 py-3 border-t border-slate-100 flex-shrink-0">
          {!discordState?.discordIntegration ? (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-blue-800">Connect Discord</h3>
