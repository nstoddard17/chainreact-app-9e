...
export function useDiscordFieldHandler({
  ...
  const handleGuildIdChange = useCallback(async (value: any, previousValue?: any) => {
    logger.info('🔍 Discord guildId changed:', { newValue: value, previousValue });
    ...
    if (value) {
      ...
      if (nodeInfo?.type?.startsWith('discord_trigger_')) {
        logger.info('🤖 Checking bot status for trigger, guild:', value);
  ...
  const handleChannelIdChange = useCallback(async (value: any) => {
    ...
    if (hasAuthorFilter) {
      logger.info('🔄 Clearing authorFilter field due to channelId change');
      setValue('authorFilter', '');
      resetOptions('authorFilter');
    ...
    if (value && values.guildId) {
      ...
      if (hasAuthorFilter && value) {
        logger.info('📥 Loading Discord users for channel:', value);
        ...
        setTimeout(() => {
          ...
          loadOptions('authorFilter', 'channelId', value, true)
            .then(() => {
              logger.info('✅ Successfully loaded Discord users for authorFilter');
            })
            .catch((error) => {
              logger.error('❌ Failed to load Discord users for authorFilter:', error);
            })
            .finally(() => {
              logger.info('🔄 Clearing loading state for authorFilter');
              setLoadingFields((prev: Set<string>) => {
                const newSet = new Set(prev);
