...
export function GenericTextInput({
  ...
  if (isAutoGeneratedField) {
    logger.debug('🧾 [GenericTextInput] Auto-generated field detected:', {
  ...
  switch (field.type) {
    case "textarea": {
      // If connect mode is enabled, render dropdown instead of textarea
      ...
      // Standard textarea mode (no connect mode)
      ...
      // Use VariableAwareTextarea when workflow context is available
      // This shows variables in user-friendly format while storing actual node IDs
      if (workflowData && currentNodeId && !textAreaOverlayActive) {
        return (
      ...
      // Fallback to standard FullscreenTextArea (no variable support)
      return (
    ...
    case "file":
      ...
      logger.debug('📎 [GenericTextInput] Passing file value to FileUpload:', {
        value,
        fileValue,
        ...
        <FileUpload
          ...
          onChange={async (files) => {
            ...
            if (files && files.length > 0) {
              ...
              logger.debug('📎 [GenericTextInput] Uploading files:', files.length);
              ...
              try {
                ...
                for (const file of Array.from(files)) {
                  logger.debug('📎 [GenericTextInput] Uploading file:', file.name);
                  ...
                  if (!response.ok) {
                    ...
                    logger.error('📎 [GenericTextInput] Upload failed:', error);
                    throw new Error(error.error || 'Failed to upload file');
                  }
                  
                  const result = await response.json();
                  logger.debug('📎 [GenericTextInput] Upload successful:', result);
                ...
                logger.debug('📎 [GenericTextInput] Setting uploadedFiles value:', {
                ...
                logger.error('📎 [GenericTextInput] Error uploading files:', error);
