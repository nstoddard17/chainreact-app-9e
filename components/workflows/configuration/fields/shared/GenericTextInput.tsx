...
export function GenericTextInput({
  ...
  const isAutoGeneratedField = Boolean(
    field.autoGenerated ||
    field.autoNumber ||
    field.airtableFieldType === 'autoNumber'
  ) && !hasUserValue
  if (isAutoGeneratedField) {
    console.log('🧾 [GenericTextInput] Auto-generated field detected:', {
  ...
  useEffect(() => {
    if (isAutoGeneratedField && hasUserValue && !autoGeneratedOverride) {
      setAutoGeneratedOverride(true)
    }
  }, [autoGeneratedOverride, hasUserValue, isAutoGeneratedField])

  const showAutoGeneratedOverlay = isAutoGeneratedField && !autoGeneratedOverride && !hasUserValue
  ...
  const renderPlaceholderOverlay = () => {
    ...
    if (showAutoGeneratedOverlay) {
      // Check if this is a truly read-only computed field (formula, rollup, etc.) that can't be overridden
      const isComputedField = field.readOnly || field.computed || field.formula ||
                             field.airtableFieldType === 'formula' ||
                             field.airtableFieldType === 'rollup' ||
                             field.airtableFieldType === 'autoNumber';

      // Determine the placeholder text based on field type
      let placeholderText = 'Automatically generated by Airtable';
      if (field.airtableFieldType === 'autoNumber') {
        ...
        // Use custom description if provided, but keep it concise
  ...
  const commonProps = {
    ...
    className: cn(
      ...
      showAutoGeneratedOverlay && "opacity-0 pointer-events-none select-none"
    ),
    readOnly: Boolean(field.readOnly) || showAiPlaceholderOverlay || showAutoGeneratedOverlay,
    ref: inputRef
  };
