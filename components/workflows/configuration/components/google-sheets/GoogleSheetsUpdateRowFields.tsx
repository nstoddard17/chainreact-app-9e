...
interface GoogleSheetsUpdateRowFieldsProps {
...
export function GoogleSheetsUpdateRowFields({
  values,
  setValue,
  ...
  currentNodeId,
  integrationId
}: GoogleSheetsUpdateRowFieldsProps) {
  ...
  }, [values.spreadsheetId, values.sheetName, externalPreviewData, fetchSheetData]);
  ...
  const getHeadingText = () => {
    if (values.rowNumber) {
      return `Update Row ${values.rowNumber}`;
    }
    return 'Update Row';
  };

  // Only show when a row is selected
  if (!values.rowNumber) {
    return null;
  }
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-slate-600 mb-4">
              Edit the values you want to update. Fields are pre-filled with current values.
            </p>
          </div>
              ...
              return (
                <div key={columnName}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <Button
                      type="button"
                      variant={connectedFields.has(`column_${columnName}`) || isVariableValue(values[`column_${columnName}`]) ? "default" : "outline"}
                      size="sm"
                      onClick={() => toggleConnectMode(`column_${columnName}`)}
                      className="h-7 text-xs gap-1.5"
                      title={connectedFields.has(`column_${columnName}`) ? "Switch to text input" : "Connect variable"}
                    >
                  ...
                  {isImageUrl ? (
                    <div>
                      {connectedFields.has(`column_${columnName}`) && workflowData && currentNodeId ? (
                        <VariableSelectionDropdown
                          workflowData={workflowData}
                          currentNodeId={currentNodeId}
                          value={values[`column_${columnName}`] || ''}
                          onChange={(val) => setValue(`column_${columnName}`, val)}
