...
import { Heart, Linkedin, Mail, Check } from 'lucide-react'
...
export function NewFooter() {
  ...
  return (
    <footer className="bg-neutral-50 dark:bg-slate-950/50 border-t border-neutral-200 dark:border-neutral-800">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
              ...
              {socialLinks.map((link) => (
                <div key={link.name} className="relative">
                  <a
                    ...
                    className={`block text-neutral-600 dark:text-neutral-400 transition-colors ${link.color}`}
                  ...
                  <AnimatePresence>
                    {link.name === 'Email' && emailCopied && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -5 }}
                        className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-neutral-900 dark:bg-neutral-700 text-white text-xs py-1 px-2 rounded whitespace-nowrap pointer-events-none"
                      >
                        Copied!
                      </motion.div>
                    )}
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h3 className="text-neutral-900 dark:text-white font-semibold mb-4">{category}</h3>
                ...
                {links.map((link) => (
                  <li key={link.name}>
                    <a
                      ...
                      className="text-neutral-600 dark:text-neutral-400 text-sm hover:text-neutral-900 dark:hover:text-white transition-colors cursor-pointer"
                    >
                      {link.name}
                    </a>
                  </li>
