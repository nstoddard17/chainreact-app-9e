...
// Side-effect import: forces Sheets' activation/deactivation/subscription
...
export async function POST(request: Request) {
  let result: Awaited<ReturnType<typeof receiveSheetsWebhook>>;
  try {
    result = await receiveSheetsWebhook(request);
  } catch (err) {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.google_sheets.receive_error",
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.google_sheets.dispatch_error",
        error: (err as Error).message,
