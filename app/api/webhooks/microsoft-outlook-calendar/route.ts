...
// Side-effect import: forces the Outlook Calendar trigger's
...
/**
 * POST /api/webhooks/microsoft-outlook-calendar
 *
 * Slice 7 plan §"Webhook receive":
 ...
 *     clientState verify + eventsGet + normalize → dispatch.
...
export async function POST(request: Request) {
  let result: Awaited<ReturnType<typeof receiveOutlookCalendarWebhook>>;
  try {
    result = await receiveOutlookCalendarWebhook(request);
  } catch (err) {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.microsoft_outlook_calendar.receive_error",
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.microsoft_outlook_calendar.dispatch_error",
        error: (err as Error).message,
...
/**
 * GET /api/webhooks/microsoft-outlook-calendar
 *
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    service: "microsoft-outlook-calendar webhook",
