...
// Side-effect import: forces Drive's activation/deactivation/subscription
...
export async function POST(request: Request) {
  let result: Awaited<ReturnType<typeof receiveDriveWebhook>>;
  try {
    result = await receiveDriveWebhook(request);
  } catch (err) {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.google_drive.receive_error",
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.google_drive.dispatch_error",
        error: (err as Error).message,
