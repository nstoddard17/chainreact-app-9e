...
// Side-effect import: forces Outlook's activation/deactivation/subscription
// registrations at module load. Without it, the receive path's
// subscription lookup would run but the renewal cron and lifecycle
// deactivate hook would be missing.
import "@/integrations/_registry";

/**
 * POST /api/webhooks/microsoft-outlook
 *
 ...
 *     clientState verify + getMessage + normalize → dispatch.
...
export async function POST(request: Request) {
  let result: Awaited<ReturnType<typeof receiveOutlookWebhook>>;
  try {
    result = await receiveOutlookWebhook(request);
  } catch (err) {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.microsoft_outlook.receive_error",
  ...
  if (result.kind === "validation") {
    // Microsoft expects the token verbatim as text/plain. Returning
    // anything else fails the handshake and the subscription create
    // call (or renewal) returns 4xx to the caller.
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        event: "webhook.microsoft_outlook.dispatch_error",
        error: (err as Error).message,
...
/**
 * GET /api/webhooks/microsoft-outlook
 *
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    service: "microsoft-outlook webhook",
