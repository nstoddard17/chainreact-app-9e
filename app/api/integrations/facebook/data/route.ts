...
export async function POST(req: NextRequest) {
  try {
    ...
    if (integrationError || !integration) {
      console.error('❌ [Facebook API] Integration not found:', { integrationId, error: integrationError })
      return NextResponse.json({
    ...
    if (integration.status !== 'connected' && integration.status !== 'active') {
      console.error('❌ [Facebook API] Integration not connected:', {
        integrationId,
    ...
    if (!handler) {
      console.error('❌ [Facebook API] Unknown data type:', dataType)
      return NextResponse.json({
    ...
    console.log(`🔍 [Facebook API] Processing request:`, {
      integrationId,
      dataType,
      status: integration.status,
    ...
    console.log(`✅ [Facebook API] Successfully processed ${dataType}:`, {
      integrationId,
    ...
    console.error('❌ [Facebook API] Unexpected error:', {
      error: error.message,
      stack: error.stack
    })
