...
function generateGoogleAuthUrl(service: string, state: string): string {
  ...
  switch (service) {
    case "gmail":
      scopes += " https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/gmail.compose https://www.googleapis.com/auth/gmail.settings.basic https://www.googleapis.com/auth/contacts.readonly"
      break
    case "google":
...
async function generateMondayAuthUrl(stateObject: any, supabase: any): Promise<string> {
  ...
  // Required scopes: me:read, boards:read, boards:write, docs:read, docs:write, workspaces:read, workspaces:write,
  // users:read, users:write, account:read, notifications:write, updates:read, assets:read, tags:read, teams:read,
  // teams:write, webhooks:write, webhooks:read
