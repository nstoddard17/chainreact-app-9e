...
export const getTrelloAllCards: TrelloDataHandler<TrelloCard> = async (integration: TrelloIntegration, options: TrelloHandlerOptions = {}): Promise<TrelloCard[]> => {
  ...
  logger.info("🔍 Trello all cards fetcher called with:", {
  ...
  try {
    ...
    if (!boardId) {
      logger.info('⚠️ No board ID provided, returning empty cards array')
      return []
    }
    ...
    logger.info(`✅ All Trello cards fetched successfully: ${cards.length} cards`)
    return cards
