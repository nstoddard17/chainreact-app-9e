...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!userId) {
      return createPopupResponse('error', provider, 'Missing userId in Google Sheets state.', baseUrl)
    }
    ...
    if (!tokenResponse.ok) {
      ...
      console.error('Failed to exchange Google Sheets code for token:', errorData)
      return createPopupResponse(
        'error',
        provider,
        errorData.error_description || 'Failed to get Google Sheets access token.',
        baseUrl,
      )
    }
    ...
    if (dbError) {
      console.error('Error saving Google Sheets integration to DB:', dbError)
      return createPopupResponse('error', provider, `Database Error: ${dbError.message}`, baseUrl)
    }

    return createPopupResponse('success', provider, 'Google Sheets account connected successfully.', baseUrl)
  } catch (error) {
    console.error('Error during Google Sheets OAuth callback:', error)
