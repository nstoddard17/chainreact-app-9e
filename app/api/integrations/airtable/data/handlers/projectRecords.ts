...
export const getAirtableProjectRecords: AirtableDataHandler<AirtableProjectRecord> = async (integration: AirtableIntegration, options: AirtableHandlerOptions = {}): Promise<AirtableProjectRecord[]> => {
  ...
  logger.info("🔍 Airtable project records fetcher called with:", {
  ...
  try {
    ...
    logger.info('🔍 Fetching Airtable project records from API...')
    ...
    for (const tableName of possibleTableNames) {
      try {
        ...
        if (response.ok) {
          ...
          logger.info(`✅ Found project table: ${tableName} with ${tableRecords.length} records`)
    ...
    if (records.length === 0) {
      logger.info('📝 No project table found, returning empty array')
    }
    
    logger.info(`✅ Airtable project records fetched successfully: ${records.length} records`)
    return records
