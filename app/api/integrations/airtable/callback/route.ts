...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    const errorHtml = `
        ...
        <html>
          <head>
            <title>Airtable Connection Failed</title>
            <style>
  ...
  try {
    ...
    if (!userId) {
      console.error("Missing userId in Airtable state")
    ...
    if (!tokenResponse.ok) {
      ...
      throw new Error(`Airtable token exchange failed: ${errorData.error_description}`)
    }
    ...
    const successHtml = `
      ...
      <html>
        <head>
          <title>Airtable Connection Successful</title>
           <style>
              body { font-family: sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; text-align: center; }
              .container { max-width: 500px; padding: 20px; border: 1px solid #ccc; border-radius: 8px; }
              h1 { color: #28a745; }
        ...
        <body>
          <div class="container">
            <h1>Airtable Connection Successful</h1>
            <p>You can now close this window.</p>
          </div>
          <script>
            if (window.opener) {
              window.opener.postMessage({ type: 'oauth-success', provider: 'airtable' }, '${baseUrl}');
              setTimeout(() => window.close(), 1000);
            }
          </script>
        </body>
