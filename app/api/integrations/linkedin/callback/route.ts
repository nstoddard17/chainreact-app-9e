  ...
  try {
    ...
    const tokenResponse = await fetch("https://www.linkedin.com/oauth/v2/accessToken", {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: `${request.nextUrl.origin}/api/integrations/linkedin/callback`,
    ...
    // Get user info from LinkedIn using the correct v2 API
    ...
    if (!userResponse.ok) {
      ...
      console.error("Failed to get user info from LinkedIn:", errorData)
    ...
    // Get email separately as it requires a different endpoint
    let email = null
    try {
    ...
    if (sessionError) {
      console.error("LinkedIn: Error retrieving session:", sessionError)
      throw new Error(`Session error: ${sessionError.message}`)
    }

    if (!sessionData?.session) {
      console.error("LinkedIn: No session found")
      throw new Error("No session found")
    }

    console.log("LinkedIn: Session successfully retrieved for user:", sessionData.session.user.id)
