    ...
    additionalIntegrationData: async (tokenData, state) => {
      try {
        ...
        if (userInfoResponse.ok) {
          ...
          logger.debug('✅ YouTube user info fetched:', {
          ...
          logger.warn('Failed to fetch YouTube user info:', await userInfoResponse.text())
          return {}
        }
      } catch (error) {
        logger.warn('Error fetching YouTube user info:', error)
        return {}
      }
    },
  })
