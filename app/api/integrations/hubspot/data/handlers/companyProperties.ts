...
export const getHubSpotCompanyProperties: HubSpotDataHandler<HubSpotProperty> = async (integration: HubSpotIntegration, options: any = {}): Promise<HubSpotProperty[]> => {
  logger.info("🔍 HubSpot company properties fetcher called with integration:", {
    id: integration.id,
  ...
  try {
    ...
    logger.info('🔍 Fetching HubSpot company properties from API...')
    ...
    logger.info(`✅ HubSpot company properties fetched successfully: ${properties.length} properties`)
    return properties
