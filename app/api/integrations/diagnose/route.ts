...
function analyzeIntegration(
  ...
  if (!tokenValid && !isDemo) {
    ...
    const hasOnlyBasicComponents =
      isTeamsProvider && availableComponents.every((comp) => comp === "teams_user_info" || comp === "teams_get_profile")

    if (!hasAnyComponents) {
      status = "⚠️ Connected but limited"
      recommendations.push("No functional components available with current permissions")
    } else if (isTeamsProvider && hasOnlyBasicComponents && unavailableComponents.length === 0) {
      // For Teams, if we only have basic components but that's all we're asking for, it's functional
      ...
      const missingImportantComponents = unavailableComponents.some((comp) => {
        const componentConfig = COMPONENT_SCOPE_MAPPING[comp]
      ...
      if (isDemo) {
        ...
        recommendations.push("Teams integration is working with available permissions")
      }
    }
  }
