 ...
 *
 * Handles the OAuth callback from Google for Google Docs integration.
...
export async function GET(request: NextRequest) {
  return handleOAuthCallback(request, {
    ...
    getRedirectUri: (baseUrl) => `${baseUrl}/api/integrations/google-docs/callback`,
    ...
    additionalIntegrationData: async (tokenData, state) => {
      try {
        ...
        if (userInfoResponse.ok) {
          ...
          logger.debug('✅ Google Docs user info fetched:', {
