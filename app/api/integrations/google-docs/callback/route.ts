...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("Google Docs callback params:", {
      hasCode: !!code,
      hasState: !!state,
      error,
    })
    ...
    if (error) {
      ...
      const errorHtml = `
        ...
        <html>
        <head>
          <title>Google Docs Connection Failed</title>
          <style>
            body { 
        ...
        <body>
          <div class="container">
            ...
            <h1>Google Docs Connection Failed</h1>
    ...
    try {
      ...
      console.log("Google Docs parsed state:", stateData)
    } catch (error) {
      console.error("Invalid state parameter in Google Docs callback:", error)
    ...
    if (!tokenResponse.ok) {
      ...
      console.error("Google Docs token exchange failed:", errorData)
      const errorHtml = `
        <!DOCTYPE html>
        <html>
        <head>
    ...
    console.log("Google Docs user info:", {
      id: userInfo.id,
    ...
    if (existingIntegration) {
      dbResponse = await supabase
        ...
        .select()
        .single()
      console.log("Updated existing Google Docs integration:", existingIntegration.id)
    } else {
      ...
      console.log("Created new Google Docs integration:", newIntegration?.id)
    }
    ...
    const successHtml = `
      ...
      <html>
      <head>
        <title>Google Docs Connected Successfully</title>
        <style>
          body { 
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
            display: flex; 
      ...
      <body>
        <div class="container">
          ...
          <h1>Google Docs Connected Successfully!</h1>
          <p>Your Google Docs account has been connected.</p>
          <p>This window will close automatically...</p>
        </div>
        <script>
          if (window.opener) {
            window.opener.postMessage({ type: 'oauth-success', provider: 'google-docs' }, window.location.origin);
          }
