...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(new URL("/integrations?error=oauth_error&provider=discord", baseUrl))
  }
  ...
  try {
    ...
    if (sessionError || !sessionData?.session) {
      ...
      return NextResponse.redirect(
        new URL("/integrations?error=session_error&provider=discord&message=No+active+user+session+found", baseUrl),
      )
    }
