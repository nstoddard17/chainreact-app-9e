 ...
 *
 * Disconnects ONE active integration row. Thin: authenticate the session → call
 * the service → map typed failures. The service (`disconnectIntegration`) is the
 * single authorization chokepoint (frozen → exact (account, id) row scope →
 * account-role + provenance) and owns the soft-disconnect + cascade + best-effort
 * revoke. The route never touches a service-role client and never sees a token or
