...
/**
 * POST /api/internal/diagnostics/workflow-readiness — thin gated shell over the
 * `diagnoseWorkflowReadiness` capability (Slice 4.MCP-STAGE-2B-3).
 *
 ...
 * membership authz, and sanitized DTO assembly live in the capability service, so
 * the future React Agent can reuse the same brain in-process. No `requireUser`
 * (no cookie); the subject is the explicit `userId`.
 */
...
export async function POST(request: Request): Promise<Response> {
  ...
  const result = await diagnoseWorkflowReadiness({ subjectUserId: userId, workflowId });
