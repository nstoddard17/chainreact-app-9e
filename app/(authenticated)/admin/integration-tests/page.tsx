...
export default function IntegrationTestsPage() {
  ...
  const loadConnectedIntegrations = async () => {
    ...
    try {
      ...
      const { data: integrations, error } = await supabase
        .from('integrations')
        .select('id, provider, status, email, account_name, username')
        .eq('user_id', user.id)
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Listen for window focus (when popup closes)
  ...
  const needsReconnection = providerIntegration?.status === 'disconnected'
