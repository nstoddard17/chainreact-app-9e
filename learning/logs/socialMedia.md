...
Enhanced the homepage How It Works section with interactive preview windows showing actual UI mockups instead of abstract shapes. The "Connect Your Apps" preview now displays a realistic integrations page with OAuth flow simulation - showing integration cards with proper descriptions, connect buttons transitioning to connected state, and a popup demonstrating Slack authorization. The "Design Your Workflow" preview shows the actual workflow builder interface with a grid canvas background, animated nodes appearing sequentially with gradient connections drawing between them, and a dragging simulation showing how new actions are added. The dashboard preview for "Activate & Relax" now shows real workflow statistics, recent executions with status indicators, and an activity graph visualization. Also fixed all integration logo display issues across light/dark modes - multi-color logos like Notion now display properly without filters, created proper colored SVGs for Google Docs, Outlook, and OneNote, and implemented seamless edge fading for the integrations showcase using CSS mask gradients that works perfectly on both edges.
