...
-- Note: Policies may already exist from later migrations, so use DROP IF EXISTS
DO $$
BEGIN
  -- Check if table has user_id or owner_id column
  IF EXISTS (
...
-- Only create organization_id index if the column exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
