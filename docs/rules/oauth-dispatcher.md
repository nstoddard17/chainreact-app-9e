...
- **Slack refresh-token reality (corrects master plan):** Slack's default OAuth v2 flow does NOT return refresh tokens. Token rotation is opt-in per Slack app config — and once enabled Slack does not allow disabling it: every token exchange then returns `expires_in` (~12 h) + a single-use rotating `refresh_token`. **SLACK-TOKEN-ROTATION-1 (2026-07):** `integrations/slack/oauth.ts` handles BOTH shapes — it persists rotation fields when present and implements a real `refreshToken()` (oauth.v2.access `grant_type=refresh_token`, dead-grant codes → `RefreshAuthRequiredError`); the manifest declares `refreshable: true`. Rows connected without rotation store null expiry/refresh-token and behave as before. (Historical: Slice 1 dropped rotation fields entirely, which turned a rotation-enabled Slack app into a recurring 12-hour reconnect loop.)
...
- **Non-refreshable providers (Discord, GitHub Apps with offline tokens, Stripe restricted keys, Notion):** Provider's `refreshToken()` is unimplemented or throws `RefreshNotSupported`. On 401, the refresh path emits `action_required` health signal immediately, no refresh attempt. (Slack moved OUT of this class in SLACK-TOKEN-ROTATION-1: its module implements a real `refreshToken()` and the manifest declares it refreshable; a Slack ROW without a stored refresh token — connected while rotation was off — still dead-ends to `action_required` via the dispatcher's "no refresh token stored" guard.)
