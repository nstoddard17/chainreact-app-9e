...
| `SMOKE_SLACK_POST_AT=<unix-seconds>` | Slack `schedule_message`, `cancel_scheduled_message` (write) | Future `post_at` for the scheduled message. Auto-computed live (~7 days out, inside Slack's 120-day window) so it never delivers mid-test and never goes stale; a pinned value wins. Unset → those two fixtures report BLOCKED_ENV. |
| `SMOKE_SLACK_UPLOAD_STORAGE_PATH=<path>` | Slack `upload_file`, `download_file` (write) | Storage path of a throwaway file the dev test stages in the `workflow-files` bucket (a self-contained v2_storage FileRef source, never an external URL). Auto-staged live via `stageSmokeFile`. Unset → those two fixtures report BLOCKED_ENV. |
...
| `SMOKE_GMAIL_ATTACHMENT_MESSAGE_ID` / `SMOKE_GMAIL_ATTACHMENT_ID` | Gmail `get_attachment` (write) | Message + attachment ids of a smoke seed email carrying one tiny text attachment. Auto-staged live via `stageGmailAttachmentMessage` (self-sends a multipart message, resolves the attachmentId, trashes the seed after the run). Unset → `get_attachment` reports BLOCKED_ENV. |
...
- **Partially covered (certified core writes; rest deferred/blocked, see notes below):**
  ...
  `microsoft-onenote` (create/update/delete_page + copy_page; create_section/create_notebook
  blocked — no delete), `notion` (create/update/archive/restore page,
...
**`microsoft-onenote:copy_page` — FIXED + CERTIFIED 2026-07-03 (`LIVE_PASS_CLEANED`).** Prior
blocker: the `copy_monitor` smoke seam captured the copied page id from the async `copyToSection`
operation's `Location`/`resourceLocation`, but OneNote returns an ephemeral/staging page id that
Graph later reports as `not found: The specified resource has been deleted`, so verify + cleanup were
non-deterministic and one run orphaned a real `crsmoke-` copy. Fix (SMOKE-ONLY): after the poll
confirms completion, the seam no longer trusts that ephemeral id. It re-lists the target smoke
section and selects the DURABLE page whose title carries the run marker AND whose id differs from
`sourcePageId` (`pickDurableCopiedPage`, a bounded list-retry that absorbs list lag; zero matches ->
keep waiting within budget, more than one -> fail fast and never guess). That durable id is captured
into the ledger for verify (`get_page_content` marker read-back) and cleanup (`delete_page`
hard-deletes both source and copy). Threaded via a new optional `AsyncCompletionSpec.config`
(token-resolved env/marker/ledger) merged into the completion seam call alongside `monitorUrl`;
absent those keys the seam falls back to the poll id, so OneDrive's copy monitor is unchanged.
Verified live TWICE consecutively: `created 2 / cleaned 2 / 0 leaked` each run, and a scoped
`crsmoke-` section sweep confirmed 0 leaked. Production `copy_page` is UNCHANGED (still returns
`{operationLocation, success:true}`; polling + discovery are smoke-only). Note: `create_page` /
`update_page` / `delete_page` each flaked their execute or verify once during the same session on
transient OneNote Graph lag but passed in another run of the batch, so their existing certs stand
(the flakiness is provider-side and unrelated to this fix, which only touches the copy path).
...
**`slack:delete_message` — CERTIFIED 2026-07-03 (`LIVE_PASS_CLEANED`), first destructive Slack
write.** Certified after Marcus reconnected Slack (the earlier `token_expired` blocker is resolved).
Flow: setup `join_channel` (the bot self-joins the public smoke channel via `channels:join` so the
verify's `conversations.history` is not `not_in_channel`; idempotent) -> `send_channel_message`
posts one `crsmoke-` message (bot-posted, capture `ts`) -> execute `delete_message` (chat.delete,
`executeIsCleanup`) -> `get_messages` INDEPENDENT read-back proves the run marker is ABSENT from
`conversations.history` (the delete echo is never trusted). Live result: `created 1 / cleaned 1 /
remaining 0` (artifact cleaned). The smoke channel auto-discovers via
`tests/smoke-actions/writeHarnessDeps/slack.ts` -> `discoverSlackSmokeChannel` (routed through
`refreshAndRetry`): picks a smoke/test/chainreact-named channel the bot is a member of OR a PUBLIC
one it can self-join (never an arbitrary channel; absent one -> BLOCKED_ENV). A `slack` branch in
the run-writes dev test overlays `SMOKE_SLACK_CHANNEL_ID`; the fixture lives in
`WRITE_SMOKE_FIXTURES`. **Capability notes for the next Slack actions:** the bot has
`chat:write.public` (can post to a public channel without joining), `channels:join`, and
