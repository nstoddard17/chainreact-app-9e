...
| Single bounded inline 429 retry (≤10s) in the wrapper | Fleetio advertises `Retry-After` explicitly; Trello's wrapper has no retry. Larger delays surface `FleetioRateLimitError` so callers/engine decide |
