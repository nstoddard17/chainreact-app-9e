...
- **LIVE in production** at `https://chainreact.app`, deploying from `v2-main` →
  ...
  **Authenticated + execution production smoke GREEN (2026-06-11)** — `npm run smoke:prod`,
  ...
  still manual.
- **Push state:** `origin/v2-main` is at `64795582a` — **deployed to prod 2026-07-04**
  (Marcus-approved pinned push `git push origin 64795582a:v2-main`: Asana live verification +
  new_task dedup fix + 3 ancestor certification commits). Deploy confirmed Ready on Vercel and
  functionally proven by the Asana live trigger retest (production route emitted the new
  timestamp-free dedup key). Local commits after that push remain unpushed. **Push posture
  unchanged:** local work stays push-gated (commit locally, don't push by default); Marcus's
  explicit per-batch approval of a verified batch authorizes a `v2-main` push **which deploys
  to prod** (no staging env yet). Approval is per-batch; it does not carry over.
- **Open threads:**
