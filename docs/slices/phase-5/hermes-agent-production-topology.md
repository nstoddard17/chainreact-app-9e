...
14. **HERMES-AGENT-APPLY-CONFIG-HINTS (next, optional)** — surface required-field HINTS on freshly
    inserted nodes (field KEY names only, never secrets/values), reusing existing node validation, plus
    an optional apply-then-save affordance. Still explicit, still no inferred config.
