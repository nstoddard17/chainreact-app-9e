...
| **In-place apply placement** — when the first added node is an ACTION and a safe anchor exists, the chain appends after it with ONE new edge (`placement: "appended"`): anchor = the selected/active node, else the SOLE chain tail. Blank graph → origin (`blank`). Ambiguous multi-tail / no anchor / trigger-first → detached side chain (`side_chain`) with a safe notice. Edges are ADD-ONLY — existing edges never removed/rewritten/split; existing config/positions never touched — HERMES-AGENT-APPLY-IN-PLACE | [`graphSlice.applyAdditivePatch`](../../../features/workflow-builder/state/graphSlice.ts) (anchor via `findChainTailId` + active node) + notice in [`WorkflowBuilder.tsx`](../../../features/workflow-builder/WorkflowBuilder.tsx) |
...
panel renders guidanceText + a preview-only "Draft preview" (or text "Suggested plan" when no preview)
  → (builder only) "Show on canvas" → BuilderPreviewOverlay renders a ghost layer over the canvas
    ...
    → "Apply preview" → planToBuilderPatch(validated plan) → graphSlice.applyAdditivePatch
        → appended after the selected node / sole tail (in-place) — else detached side chain (notice)
...
13. **HERMES-AGENT-APPLY-INSERT-BETWEEN (next)** — true insertion BETWEEN two selected nodes (split one
    existing edge, rewire A→new→B) — the first edge-rewrite operation, still explicit, still no delete/
    save. A separate slice could add config-prefill assistance (never secrets) + an apply-then-save
    affordance.
