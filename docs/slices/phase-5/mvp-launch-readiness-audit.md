...
> **STATUS RECONCILED 2026-07-03 — read
> [`mvp-launch-status-reconciliation.md`](./mvp-launch-status-reconciliation.md) first.**
> Several findings below are now **decided or fixed** and should not be treated as open:
> (1) **React Agent ships visible; Hermes + the deterministic "Check workflow" checker are
> expected launch features** — the "AI rail on/off?" go/no-go is decided, not open
> (Builder/AI row + Exec-summary #4 + must-fix #5). (2) **Staging is an accepted
> production-first risk, intentionally deferred by owner decision** — a plan exists
> ([`staging-environment-plan.md`](./staging-environment-plan.md)); it is not an active next
> task (blocker row 1 + must-fix #1). (3) **Dedup outage now fails CLOSED** (commit
> `19c00455f`, already annotated in the Execution row). (4) **Billing is code-ready pending
> Marcus dashboard verification** (commit `141fd5789`,
> [`billing-production-readiness-closeout.md`](./billing-production-readiness-closeout.md)).
> (5) Live-provider validation is owned by a **separate chat**. The audit body below is left
> as originally written for the record; the reconciliation doc is the current status.
...
| Builder / AI | React Agent rail is **gated off by default** (`HERMES_AGENT_ENABLED` unset → whole rail collapses to an "unavailable" note). The free, LLM-free deterministic "Check workflow" pill is nested inside the gated panel and hidden with it. | Product decision: launch with no visible builder AI (fine, but the "React Agent" pitch is invisible) OR turn it on (lights up model spend, credit-gated). Either way the free validation-in-chat is currently hidden. | `services/ai-guidance/gateway/gatewayConfig.ts:54`; `features/workflow-builder/panels/BuilderGuidanceRail.tsx:123-185`; `WorkflowGuidancePanel.tsx:258-266` | Product decision + tiny slice: optionally decouple deterministic "Check workflow" from the Hermes flag so it's available with AI off. | Medium | **RECONCILED 2026-07-03 — DECIDED, NOT A BLOCKER.** Owner decision: React Agent ships **visible**; Hermes + the deterministic checker are **expected launch features**. OFF is only a safe code default; enabling Hermes in prod is env config (like Stripe keys), not code. Code does not contradict this (rail renders when `guidanceEnabled && accountId`). The checker/Hermes coupling stays an optional decouple, not a launch item. See `mvp-launch-status-reconciliation.md`. |
...
| Docs / Ops | CLAUDE.md and parts of the roadmap describe the **V1 billing/cron model** (report-overage, usage-alerts, clean-session-side-effects, reset-task-usage). V2 ships a different reserve/reconcile + `account_billing` + AI-credits model; those crons correctly do not exist. | Stale instructions cause future audits/engineers to chase phantom infrastructure and mis-assess billing readiness. | `services/observability/cronExpectations.ts:17-26` (actual `MONITORED_CRONS`); CLAUDE.md §"Task Cost Visibility & Billing" (V1 model) | Docs slice: reconcile CLAUDE.md/roadmap billing+cron sections to the V2 reserve/reconcile model. | Low | **PARTLY CORRECTED 2026-07-03.** The **V2** `CLAUDE.md` has no such billing/cron section (that lives in the V1 repo's CLAUDE.md). The stale V1 content in V2 is in the **roadmap** Phase 7, now banner-corrected as superseded by the reserve/reconcile model. V2 `MONITORED_CRONS` contain no billing/overage crons. See `mvp-launch-status-reconciliation.md`. |
