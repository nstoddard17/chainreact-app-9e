...
2. **The field-level cascade (`base → table → field`) is BLOCKED by the current builder.** A `fields` / `views` / `attachment_fields` resolver needs **two** deps (`baseId` + `tableIdOrName`), but the builder's cascade ([`SchemaForm.tsx:173`](../../../features/workflow-builder/config-modal/SchemaForm.tsx) builds `deps = { [field.dependsOn]: parentValue }`) is **single-parent only** (Slice 3.33; [`fields/types.ts:31`](../../../features/workflow-builder/config-modal/fields/types.ts)). So field-level pickers are **deferred** — hand-typed field names are acceptable for v1 (and the record-write path is paste-JSON anyway). See §3.
...
| `airtable:fields` | `list_records.fields` (multiselect), `create/update_record.fields` keys (paste-JSON; no direct consumer), `add_attachment.fieldName` | reuse `basesGetSchema` → find table by `tableIdOrName` → return `table.fields` as `{value: field.name, label: field.name, description: field.type}` | `["baseId", "tableIdOrName"]` | **DEFER (blocked)** | **Yes** — field names are human-readable; `string-array` / paste-JSON / text work without it |
| `airtable:views` | `list_records.view` | reuse `basesGetSchema(includeViews:true)` → table.views | `["baseId", "tableIdOrName"]` | **DEFER (blocked)** | Yes — view name is typeable |
| `airtable:attachment_fields` | `add_attachment.fieldName` (filtered to attachment-type fields) | reuse `basesGetSchema` → table.fields filtered to `type === "multipleAttachments"` | `["baseId", "tableIdOrName"]` | **DEFER (blocked)** | Yes — `fieldName` text acceptable |
...
**The blocker, concretely:** every field-level resolver (`fields` / `views` / `attachment_fields` / `records`) needs **two** deps (`baseId` + `tableIdOrName`). The route + client support multi-dep (`?deps[baseId]=…&deps[tableIdOrName]=…`; [`app/api/options/[source]/route.ts`](../../../app/api/options/[source]/route.ts), [`lib/api/options.ts`](../../../lib/api/options.ts)), **but the builder's `SchemaForm` cascade only populates the single `dependsOn` parent** ([`SchemaForm.tsx:158-173`](../../../features/workflow-builder/config-modal/SchemaForm.tsx); [`fields/types.ts:31`](../../../features/workflow-builder/config-modal/fields/types.ts) — "single-parent only"). A field declaring `dependsOn: "tableIdOrName"` would send `{tableIdOrName}` but NOT `baseId`, so the resolver's `requiredDeps` check returns `MISSING_DEPENDENCY`. → **These resolvers are not usable until a multi-parent `dependsOn` lands in the builder** (a separate builder-infra slice). Until then they would be dead code, so we **do not build them in this arc.**

**`add_attachment` special-case (brief asked to call this out):** the ideal UX is a `fieldName` combobox backed by `airtable:attachment_fields` (dep `baseId` + `tableIdOrName`). That's the **2-dep blocker** above → **deferred**. v1 ships `fieldName` as **text** (hand-typed attachment-field name; the handler forwards it verbatim and Airtable validates). No dedicated attachment-field resolver in this arc.
...
**Recommendation:** build `airtable:bases` + `airtable:tables` in META-2 (`bases` = one new `basesList` helper). Defer `fields` / `views` / `attachment_fields` (blocked by single-parent cascade). Reject `records` for v1. Both shipped resolvers are read-only against scopes **already in the manifest** (`schema.bases:read`) → **no manifest/scope change, no reconnect.**
...
- **Field-level resolvers (`fields` / `views` / `attachment_fields`) → DEFER.** Blocked by the single-parent builder cascade (§3); hand-typed names acceptable; unblocked by a future multi-parent `dependsOn` builder slice.
...
| **AIRTABLE-META-2** | 2 resolvers + the new `basesList` helper + resolver tests | new `integrations/airtable/api/bases.ts` export `basesList` (or new `integrations/_shared/airtable/api/` helper) for `GET /v0/meta/bases`; `integrations/airtable/options/{bases,tables}.ts`; register in `services/options/_registry.ts`; resolver unit tests (mock the Airtable boundary) |
...
**Why 3 slices (same shape as Excel), not the brief's hypothetical 5:** the field-level resolvers are blocked (§3), so META-2 ships only the 2 wireable resolvers (`bases` + `tables`) + one helper — small and fast. With those in place, META-3 combines all 11 ActionMeta + the single TriggerMeta + the sub-registry + the COVERED flip in one slice (Excel proved the combined actions+triggers+flip shape with 10 actions + 5 triggers; Airtable's 11 actions + 1 trigger is comparable). No separate "META-4 trigger" or "META-5 flip" is warranted because there's exactly one trigger and the flip is one line. A later **AIRTABLE-META-4** can add the deferred field-level resolvers **once** the builder gains multi-parent `dependsOn`.

---

## 7. Tests required

- **Resolver tests (META-2):** `airtable:bases` / `airtable:tables` return mapped `{value,label}` items (`tables` value = table id); `tables` `requiredDeps:["baseId"]` short-circuits (`MISSING_DEPENDENCY`); stale parent base → empty items (cascade fallback, not error); provider 4xx → `OptionsResolverError("PROVIDER_ERROR")`; 401 → `INTEGRATION_DISCONNECTED`; **no token/raw-body leakage**; **Airtable boundary mocked — no real API calls**.
...
- [ ] `airtable:bases` + `airtable:tables` resolvers exist (`basesList` helper added); the field-level resolvers (`fields` / `views` / `attachment_fields`) are explicitly **deferred with the single-parent-cascade rationale**, and `records` is **rejected** for v1;
...
1. **Single-parent builder cascade (HARD BLOCKER for field-level pickers).** `SchemaForm` sends only the `dependsOn` parent's value. `fields`/`views`/`attachment_fields` resolvers (2-dep) are deferred until a multi-parent `dependsOn` builder slice. Mitigation: hand-typed field/view names + paste-JSON record fields are fully functional for v1.
