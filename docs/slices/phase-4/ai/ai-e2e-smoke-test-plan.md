...
| Service enrichment | [`enrichRequiredUserInputs.test.ts`](../../../../tests/unit/services/ai/planner/enrichRequiredUserInputs.test.ts) (8 cases) | `slack:send_channel_message.channel` → enriched with `combobox` + `slack:channels` + `allowFreeText: true`; `slack:send_channel_message.text` → enriched with `textarea`; no-field / unresolvable nodeId / unknown field / null patch all pass through unchanged; no-leak (a patch with `accessToken: "xoxb_REDACTED_EXAMPLE"` — a deliberately non-token-shaped placeholder, so no literal `xoxb-…` pattern lives in the repo — produces no leak in serialized output). |
...
| Ambiguity + required-field completeness (AI-33) | [`enrichRequiredUserInputs.test.ts`](../../../../tests/unit/services/ai/planner/enrichRequiredUserInputs.test.ts) (9 derivation cases) + [`buildWorkflowPlanPrompt.test.ts`](../../../../tests/unit/services/ai/planner/buildWorkflowPlanPrompt.test.ts) (ambiguity/content/null-patch rule cases) + [`buildWorkflowPlanPromptV2.test.ts`](../../../../tests/unit/services/ai/planner/buildWorkflowPlanPromptV2.test.ts) (R1/R3/R7 grouping) + [`planWorkflowFromPrompt.test.ts`](../../../../tests/unit/services/ai/planner/planWorkflowFromPrompt.test.ts) (6 orchestrator cases) | Fixes two live bugs from "When I get an email send a Slack message": (1) generic "email" was silently assumed to be Gmail; (2) the Slack message text was never asked. **R1 ambiguous-category rule:** generic capability words ("email"/"calendar"/"drive"/"chat"/"payment") must NOT resolve to a specific provider when ≥2 catalog providers match + the user didn't name one — set proposedPatch null + ask which provider; never default email→Gmail / calendar→Google Calendar / drive→Google Drive / chat→Slack. Naming the provider resolves it. **R3 content-field rule:** unspecified free-text content (message/email body) must be asked, not invented; AI_FIELD only for asked-for summary/generated or upstream-derived content. **R7 null-patch completeness:** when proposedPatch is null but nodes are intended, list every missing required field, not just the blocker. **Service-side `deriveMissingRequiredFieldInputs`:** derives a question for every required field the model left empty in the patch (literal/ref/AI_FIELD = filled = not derived), merged + enriched + drives the apply-gate (apply blocked until answered). No UI change — derived `channel`→`slack:channels` combobox, `text`→textarea, both via AI-22's `RequiredInputControl`. No narrowing-cost / provider-metadata / execution / billing change; no-substitution strengthened; full-catalog fallback intact; `PLANNER_PACKET_VERSION` unchanged. Manual live re-test: re-run "When I get an email send a Slack message" → expect a "which email app?" question + a "what should the Slack message say?" question, no silent Gmail. |
...
- **Stripe failed-payment as a happy-path apply scenario.** Out of scope until the provider track ships [`stripe:event_received` TriggerMeta](../providers/stripe-trigger-meta-plan.md). Stripe failed-payment remains a S3 / S4 (needs-input / unsupported) scenario until then. Do NOT use it as a happy-path smoke target.
...
### Chat composer QoL (Slice 4.REACT-AGENT-CHAT-QOL-1)

70. **Enter sends / Shift+Enter wraps** — type a prompt and press **Enter** → it plans (`POST /ai/plan`); type a multi-line prompt using **Shift+Enter** for line breaks, then Enter to send → newlines preserved, single submit. Empty / whitespace-only Enter does nothing; pressing Enter again while "Thinking…" does not double-submit.
71. **Inline Send details** — ask for something that needs details ("post a Slack message when a new email arrives") → fill the channel combobox + message control → the **Send details** button directly under the controls enables → click it WITHOUT scrolling to the bottom composer → it submits exactly like the bottom button (`POST /ai/complete` for a deterministic fill, or `/ai/plan` on a free-text/correction) with no duplicate call or duplicate message. Enter pressed inside the message/combobox control does NOT submit (it edits the field). Reopening the thread shows the earlier required-input turn as read-only with no active Send details button.
...
- **Slice 4.REACT-AGENT-CHAT-QOL-1 (2026-05-29)** added chat-composer quality-of-life scenarios (Enter submits / Shift+Enter newline / whitespace + pending never submit; an inline "Send details" button under the active required-input controls that shares the bottom composer's submit path with no duplicate call; Enter inside a control doesn't submit; only the latest required-input block shows an active button). Coverage map + manual steps in [`react-agent-live-qa-matrix.md`](../react-agent-live-qa-matrix.md) §"Slice 4.REACT-AGENT-CHAT-QOL-1" (Q1–Q10 + manual steps 21–22) and §5 manual checklist 70–71. UI/UX-only — no planner / routing / execution / billing / metadata change.
- **Slice 4.BUILDER-APPLY-HYDRATE-RACE-1 (2026-05-29)** added post-apply hydrate-stability scenarios (a stale/older-revision hydrate must never overwrite a freshly-applied graph; the applied canvas survives config inspection without a refresh). Coverage map + manual steps in [`react-agent-live-qa-matrix.md`](../react-agent-live-qa-matrix.md) §"Slice 4.BUILDER-APPLY-HYDRATE-RACE-1" (H1–H5 + manual steps 19–20). Note: browser-extension console errors are not an app cause unless reproduced without extensions.
