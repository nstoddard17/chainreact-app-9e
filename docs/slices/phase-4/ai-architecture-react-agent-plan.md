...
| **AI-34A** | shipped | Second real model adapter — OpenAI Responses-API (`services/ai/modelClients/openaiClient.ts`) + `OPENAI_MODELS` registry (`gpt-4.1` / `gpt-4.1-mini`) + provider-aware resolver (`getModelForProviderTier`) + factory routing (`createModelClientForModel` handles `provider==="openai"`) + `isOpenAiProviderEnabled()` flag (default off). NO behavior switch — default planner stays Anthropic/Sonnet 4.6; nothing routes to OpenAI. See [`openai-adapter-setup-and-audit.md`](./ai/openai-adapter-setup-and-audit.md). See note below. |
...
| **AI-36** | shipped | **OpenAI-only planner routing; Anthropic disabled at runtime.** The React Agent planner routes to OpenAI `gpt-4.1-mini` via `createPlannerModelClient` (gated on `ENABLE_OPENAI_PLANNER` + `ENABLE_OPENAI_PROVIDER` + `OPENAI_API_KEY`). **Anthropic is NEVER called** unless the explicit emergency flag `ENABLE_ANTHROPIC_PLANNER_FALLBACK=true` is set; there is NO silent fallback. OpenAI parse/rate-limit/provider/network/not-configured failures surface the existing model-unavailable flow (no Anthropic). Anthropic code remains available but dormant. All safety gates unchanged (parser / WorkflowPatchSchema / AI-5 preview / apply-readiness / no-substitution / deterministic completion). Live-verified: 5 smoke prompts all `provider=openai, model=gpt-4.1-mini`. See note + [`openai-adapter-setup-and-audit.md`](./ai/openai-adapter-setup-and-audit.md). |
| **4.BUILDER-APPLY-HYDRATE-RACE-1** | shipped | **Post-apply canvas state stability.** Fixed the bug where a React Agent Apply succeeded + persisted, but the canvas reset to empty until refresh. Root cause: two hydrate sources with no freshness ordering — `WorkflowBuilder`'s prop-effect (`hydrate(workflow.id, workflow.draftDefinition)`, where the server prop is STALE/empty for a freshly-created workflow, and whose cleanup `reset()`s) and `BuilderAiPanel.onApplied` (`getWorkflow` → `hydrate(newDraft)`). A late prop-driven re-hydrate (or the effect's cleanup reset on a same-id re-run) clobbered the applied graph. Fix: (1) `graphSlice.hydrate(workflowId, def, revision?)` gains a **revision guard** — a hydrate for the SAME workflow carrying a STRICTLY-OLDER `updatedAt` is ignored (`hydratedRevision` tracks the last accepted; compared as epoch ms; unknown revisions accept → legacy-safe; a different workflow always hydrates). (2) `WorkflowBuilder`'s single hydrate+reset effect is SPLIT: a guarded hydrate effect (deps incl. `workflow.updatedAt`, no cleanup) + an id-keyed reset effect (resets graph/config/run ONLY on workflow-id change or unmount, never on a same-id re-render). (3) `onApplied` threads `detail.updatedAt` so the fresh hydrate wins. No arbitrary timeouts, empty-state not hidden — state ownership fixed. NO planner-routing / provider-metadata / execution / billing change; planner stays OpenAI (Anthropic not called). Note: the reported `chrome-extension://` / Grammarly console errors were browser-extension noise, not the cause. Tests: graphSlice revision-guard suite (stale/newer/equal/different-workflow/legacy/reset + the post-apply race), a WorkflowBuilder stale-prop-re-render test, a BuilderAiPanel post-apply hydrate+revision test. See note + [`react-agent-live-qa-matrix.md`](./react-agent-live-qa-matrix.md). |
| **4.PROVIDER-CATALOG-INTEGRITY-1** | shipped | **Full catalog exposure audit + malformed-key fix + friendly unsupported-node copy.** Root-caused a live `UNKNOWN_TRIGGER: 'gmail:gmail:new_email'` failure: Gmail "new email" IS a real registered trigger (`gmail:new_email`); the planner emitted a self-qualified node (`provider:"gmail"`, `type:"gmail:new_email"`) so `checks.ts` built the doubled lookup key. (1) **Normalize:** new pure [`normalizeAiPatchNodeKeys`](../../../services/ai/patch/normalizeAiPatchNodeKeys.ts) strips a redundant `<provider>:` prefix from `addNode`/`replaceTrigger` `type` at the AI ingestion boundary (preview + apply, before validation), so a supported node resolves against its real key + persists the real dispatch `type`. STRICT — only strips a prefix equal to the node's own provider; genuinely-unknown nodes still reject (validation NOT weakened). Plus a GOOD/BAD node-shape example in `PATCH_SHAPE_GUIDE`. (2) **Friendly copy:** new pure [`humanizePatchError`](../../../core/errors/humanizePatchError.ts) maps `UNKNOWN_TRIGGER`/`UNKNOWN_ACTION`/`UNKNOWN_NODE` to user copy with no raw/doubled key, no "registered", no "V2" (raw stays as `devDetail`); applied in `previewWorkflowPatch` (errors + `blockedReason`) + `applyWorkflowPatch`; the preview builds a best-effort subject (`Gmail 'New Email'`) via `formatTypeKey`. No UI change (renders `e.message`/`blockedReason`). (3) **Integrity guard:** new [`tests/structure/ai-catalog-consistency.test.ts`](../../../tests/structure/ai-catalog-consistency.test.ts) asserts the planner catalog ⊆ the validated registry (every key resolves via `getActionMeta`/`getTriggerMeta`), keys are well-formed (exactly one colon, provider-prefixed, no `provider:provider:type`), and `gmail:new_email` is exposed — complementing the existing handler↔meta `discovery-meta-coverage.test.ts`. Audit found NO launch-scope metadata gaps; the catalog is registry-driven (can't invent nodes). Packet version unchanged. NO validation weakening, NO fake metadata, NO preview/apply bypass, NO billing; planner stays OpenAI (Anthropic not called). See note + [`react-agent-live-qa-matrix.md`](./react-agent-live-qa-matrix.md). |
...
| **4.REACT-AGENT-CHAT-QOL-1** | shipped | **Chat composer Enter-to-send + inline "Send details" button.** UI/UX-only polish; no planner / routing / execution / billing / metadata change. **(A) Enter-to-send:** the chat composer ([`_BuilderAiPanelComposer`](../../../features/workflow-builder/panels/_BuilderAiPanelComposer.tsx)) `Textarea` gains an `onKeyDown` — plain Enter submits (calls the existing `onSubmit`), Shift+Enter inserts a newline, IME-composition Enter is ignored (CJK/diacritic safety), and the submit is gated on the existing `canSubmit` (whitespace-only / too-long / busy never submit, so no double-submit). The kbd hint changed from `⌘ ↵` to `↵ send · ⇧↵ newline`. The required-input controls (text/textarea/combobox/select/number) are deliberately NOT given Enter-to-submit — Enter there stays a newline / no-op so the user relies on the inline button (and combobox keyboard nav is untouched). **(B) Inline Send details:** [`RequiredInputControlsBlock`](../../../features/workflow-builder/panels/_BuilderAiPanelRequiredInputs.tsx) renders a `builder-ai-send-details-inline` button directly under the interactive controls (only when `controls.length > 0` and the panel wired the handler), so the user doesn't scroll to the bottom composer after filling a channel/message. It calls the **same** `BuilderAiPanel.handleSubmit` as the bottom composer button (one submit path — no duplicate `/ai/complete` or `/ai/plan` call, no duplicate message), disabled until there's something to send / while busy (`canSubmitDetails` mirrors the composer's `canSubmit`). **(C/D) Historical safety:** the controls block only mounts for the ACTIVE (latest, non-persisted) `plan_result` — `PlanResultBody` collapses older turns to a summary and persisted plan_results synthesize empty `requiredUserInput` — so older / persisted required-input messages never show an active inline button. Tests: [`BuilderAiPanel.chatQol.test.tsx`](../../../tests/unit/features/workflow-builder/panels/BuilderAiPanel.chatQol.test.tsx) (13) — Enter submits / Shift+Enter newline / whitespace no-submit / pending no-double-submit / button still works / Enter-in-control no-submit / inline appears + disabled-until-staged / inline submits staged answer with no duplicate call / inline ≡ bottom path / combobox select + inline submit (+ Enter doesn't break combobox) / only-latest-block-has-button / persisted-thread-no-button / single answer bubble. NO planner provider routing change (OpenAI stays the planner per AI-36; Anthropic not called) / no execution / billing / provider-metadata / general-help change / no graph mutation before Apply. See [`react-agent-live-qa-matrix.md`](./react-agent-live-qa-matrix.md). |
...
**Deliverable** — [`ai-e2e-smoke-test-plan.md`](./ai/ai-e2e-smoke-test-plan.md):
...
**Live follow-up smoke status.** PENDING — this environment has no `ANTHROPIC_API_KEY` and no running dev server attached to the assistant, so the planner round-trip cannot be exercised end-to-end from here. Marcus runs the manual smoke per the steps in [`ai-e2e-smoke-test-plan.md`](./ai/ai-e2e-smoke-test-plan.md) §5.1 (steps 6 → 7e — ambiguous prompt → follow-up → multi-turn → Clear → chat-layout sanity). The expected behavior is documented exhaustively in the smoke plan; the AI-21B regression suite + this refactor's zero-regression verification (1306 tests across the AI + workflow-builder sweep) is the automated coverage backstop.
...
### Slice 4.BUILDER-APPLY-HYDRATE-RACE-1 — Post-apply canvas state stability

**(2026-05-29).** Live bug: "Change the trigger to a new Gmail email" → Apply succeeded ("Applied 1 change to 'A3'…"), nodes appeared, then the canvas reset to EMPTY while the user inspected config; a refresh restored them — i.e. the server persisted correctly but a stale client hydrate overwrote the applied graph.

**Root cause.** Two hydrate sources with no freshness ordering:
- `WorkflowBuilder` ([WorkflowBuilder.tsx](../../../features/workflow-builder/WorkflowBuilder.tsx)) hydrated from the **server prop** `workflow.draftDefinition` in an effect whose deps included `workflow.draftDefinition` and whose **cleanup called `reset()`**. For a freshly-created workflow the prop is empty; any re-run of that effect (stale prop re-render, Strict-Mode/Fast-Refresh remount) reset → hydrated empty.
- `BuilderAiPanel.onApplied` ([BuilderAiPanel.tsx](../../../features/workflow-builder/panels/BuilderAiPanel.tsx)) hydrated the FRESH post-apply draft via `getWorkflow`.
Whichever resolved last won; the stale/empty one could clobber the applied graph. (`AiApplySuccess` returns only metadata, so the client must re-fetch + re-hydrate — that part is correct; the missing piece was a freshness guard.)

**Fix (state ownership / freshness guard — no timeouts, empty state not hidden):**
1. **Revision guard** in `graphSlice.hydrate(workflowId, def, revision?)` ([graphSlice.ts](../../../features/workflow-builder/state/graphSlice.ts)): new `hydratedRevision` tracks the `updatedAt` of the last accepted hydrate; a same-workflow hydrate with a **strictly-older** revision is ignored (`isStaleRevision` compares epoch ms; unknown/unparseable revisions are not stale → legacy 2-arg callers accept unchanged; a different `workflowId` always hydrates). `reset()` clears `hydratedRevision`.
2. **Effect split** in `WorkflowBuilder`: the prior single effect (hydrate + reset, cleanup `reset()`, deps incl. `draftDefinition`) became a **guarded hydrate effect** (deps `[workflow.id, workflow.draftDefinition, workflow.updatedAt, hydrate]`, NO cleanup) + an **id-keyed reset effect** (deps `[workflow.id, …]`) that resets graph/config/run only on workflow-id change or unmount. A same-id re-render no longer resets the graph, and a stale prop re-hydrate is dropped by the guard.
3. `onApplied` passes `detail.updatedAt` as the revision so the post-apply hydrate is authoritative.

**Invariant.** After Apply succeeds, the applied graph is the canvas state; a stale (older-revision) hydrate must never overwrite it; saved + pending reflect the applied draft; refresh shows the same graph. **Browser-extension console errors (`chrome-extension://`, Grammarly, "a listener indicated an asynchronous response…") are not an app cause** unless reproduced without extensions — they were noise here.

**Boundaries.** No planner-routing / provider-metadata / workflow-execution / billing change; planner stays OpenAI (Anthropic not called); no general app-help. Gates: tsc / eslint(`--max-warnings=0`) / structure / migrations clean; full sweep 14,632 passed / 17 skipped / 0 failed.

### Slice 4.PROVIDER-CATALOG-INTEGRITY-1 — Catalog exposure audit + malformed-key fix + friendly unsupported copy

**(2026-05-29).** Triggered by a live `UNKNOWN_TRIGGER: 'gmail:gmail:new_email'` preview failure.

- **Audit / Gmail truth:** Gmail "new email" is a real, registered, planner-visible trigger — exact key `gmail:new_email` ([`integrations/gmail/triggers/newEmail/newEmail.meta.ts`](../../../integrations/gmail/triggers/newEmail/newEmail.meta.ts), in `ALL_TRIGGER_META` + discovery + `getProviderCatalog`). The trigger was NOT missing. The planner put the combined catalog key in `node.type`, so [`checks.ts`](../../../services/workflows/patch/checks.ts) built `${provider}:${type}` = `gmail:gmail:new_email` → unresolved → UNKNOWN_TRIGGER. The error message renders the raw doubled key + "registered" + "V2" — bad UX. No launch-scope metadata gaps: handler↔meta is enforced for all `COVERED_PROVIDERS` by `discovery-meta-coverage.test.ts`, and the catalog is built off the same registry so it cannot invent nodes.
- **Fix 1 — normalization (chosen over prompt-only):** [`normalizeAiPatchNodeKeys`](../../../services/ai/patch/normalizeAiPatchNodeKeys.ts) (pure) strips a redundant `<provider>:` prefix from `addNode`/`replaceTrigger` `type` at the AI ingestion boundary — wired into both `previewWorkflowPatch` and `applyWorkflowPatch` BEFORE validation (in apply, before `materializeAiPatchNodeIds`). STRICT: only strips a prefix exactly equal to the node's own provider, so a mismatched prefix (`gmail` + `outlook:x`) and a genuinely-unknown node still reject — validation is NOT weakened; it only resolves the unambiguous "didn't split the catalog key" case to the node the model clearly intended. `PATCH_SHAPE_GUIDE` gains a GOOD/BAD example (defense in depth; the normalization is the reliable fix).
- **Fix 2 — friendly copy:** [`humanizePatchError`](../../../core/errors/humanizePatchError.ts) (pure, parallel to `humanizeActionError`) maps `UNKNOWN_TRIGGER`/`UNKNOWN_ACTION`/`UNKNOWN_NODE` → `{message, devDetail}` with no raw key, no "registered", no "V2"; other codes pass through. Applied server-side in `previewWorkflowPatch` (rewrites `errors[].message` + sets `blockedReason` to the humanized message, dropping the `${code}:` prefix) and `applyWorkflowPatch` (`toUserFacingErrors` = scrub then humanize). The preview builds a best-effort subject (`Gmail 'New Email'`) from the failing node via `formatTypeKey`, stripping any self-prefix so a doubled key never reaches user copy. The raw message stays as `devDetail`; the error `code` is preserved for dev. No UI change — `_BuilderAiPanelPreview` already renders `e.message` + `blockedReason`.
- **Fix 3 — integrity guard:** [`tests/structure/ai-catalog-consistency.test.ts`](../../../tests/structure/ai-catalog-consistency.test.ts) asserts the planner catalog ⊆ the validated registry (every action/trigger key resolves via the SAME `getActionMeta`/`getTriggerMeta` from `@/services/discovery/_registry` that `checks.ts` uses), every key is well-formed (exactly one colon, provider-prefixed, never `provider:provider:type`), and `gmail:new_email` is exposed.
- **Unsupported behavior:** a genuinely-unknown node still rejects at preview/apply (validation strict) and the planner already returns null patch + `unsupportedRequests` + no substitution (existing R1 / AI-24 / AI-33 rules) — now surfaced with friendly copy.
- **Boundaries.** No validation weakening; no fake metadata; no `WorkflowPatchSchema`/preview/apply bypass; execution semantics changed only by correcting a malformed node to an already-supported runtime handler; no billing; planner stays OpenAI (Anthropic not called); no general app-help assistant. Gates: tsc / eslint(`--max-warnings=0`) / structure / migrations clean; full sweep 14,623 passed / 17 skipped / 0 failed.
...
**AI-34A (2026-05-28).** Added OpenAI as a *supported, wired, tested* provider — the second real adapter — behind `ENABLE_OPENAI_PROVIDER` (default off). `OPENAI_MODELS` (`gpt-4.1` strong / `gpt-4.1-mini` fast) in [`core/ai/models.ts`](../../../core/ai/models.ts), provider-aware resolver `getModelForProviderTier`, cross-provider `getModelById` (telemetry), the Responses-API adapter [`openaiClient.ts`](../../../services/ai/modelClients/openaiClient.ts), and factory routing in [`createModelClient.ts`](../../../services/ai/modelClients/createModelClient.ts). **No behavior switch** — the default planner path uses `getModelForTier`/`getModelForFeature` (Anthropic) and never resolves an OpenAI model. Full setup + compatibility audit in [`openai-adapter-setup-and-audit.md`](./ai/openai-adapter-setup-and-audit.md).
