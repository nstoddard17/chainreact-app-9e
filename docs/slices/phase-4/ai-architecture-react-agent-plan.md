...
| **AI-12C** | shipped | Planner JSON-only response hardening. Fixes the remaining `parse/NOT_JSON` (model returning prose / a ```json fence instead of a bare JSON object): the Anthropic adapter now seeds an assistant `{` prefill (forcing a JSON-object start; provider strips it, adapter re-attaches with a double-`{` guard) and the prompt gains an explicit JSON-only rules block (exactly one object, first `{`/last `}`, no fences/prose/comments/trailing commas) rendered last. Parser stays strict (NO broad JSON extraction). Builder shows a NOT_JSON-specific safe message. See note below. |
...
After AI-12B fixed the patch *shape* (INVALID_PATCH), real testing still hit `parse/NOT_JSON` for the same prompt — the model was returning output that isn't a bare JSON object (a preamble like "Sure, here's the plan:", or a ```json fence), which `parseWorkflowPlanResponse` rejects at `JSON.parse` — *before* patch validation or preview. This is the dominant remaining planner parse failure; it is a model-output-format problem, not a schema or registry problem. **The raw completion is not logged (by design), so the exact offending text isn't directly observable — but the `NOT_JSON` code means the model call succeeded and the output failed JSON parsing, and preamble/fence is the overwhelmingly common cause.** Fix is two coordinated, narrow changes; the parser was NOT loosened and NO broad JSON-substring extractor was added (that would mask the secret-refusal / shape guarantees).

- **[`anthropicClient.ts`](../../../services/ai/modelClients/anthropicClient.ts) — assistant `{` prefill.** The adapter appends a trailing `{ role: "assistant", content: "{" }` turn, so the model *structurally* begins its completion inside a JSON object — it cannot emit a conversational preamble or a code fence. The Messages API returns only the continuation (never the prefill), so `reattachJsonPrefill` prepends `{` back — with a guard that skips the prefix when the continuation already starts with `{` (so a full-object response, e.g. a test mock, isn't corrupted into `{{…`). Prefill is applied only when the conversation ends on a user turn (the planner is always `[system, user]`); a future multi-turn caller ending on an assistant turn degrades to prompt-only (two assistant turns is an illegal request). The blank-continuation check runs before re-attach, so an empty completion is still `EMPTY_RESPONSE`, not a lone `{`. This is the primary, robust fix — it removes the preamble class entirely rather than trying to parse around it.
- **[`buildWorkflowPlanPrompt.ts`](../../../services/ai/planner/buildWorkflowPlanPrompt.ts) — JSON-only instruction.** Constraint #3 strengthened (exactly one object, first `{` / last `}`, no fences/comments/trailing commas) and a dedicated `JSON_OUTPUT_RULES` block rendered **last** (recency) restating: one object only, first/last char, no markdown/fence, no prose before/after, no comments/trailing commas, and "if unsure, return `proposedPatch:null` + `requiredUserInput`". Belt-and-suspenders to the prefill.
...
- **Parser unchanged.** `parseWorkflowPlanResponse` keeps its existing behavior: trim, strip ONE fully-wrapping code fence, then strict `JSON.parse` (rejects prose before/after, comments, trailing commas). No change — AI-12C only adds tests that lock these.
- **Stripe/Slack (unchanged, out of scope):** once JSON-only holds, the Stripe-DM prompt still degrades to needs-input/unsupported (a **200**, not a 502) because `stripe:event_received` has no `TriggerMeta` in the catalog — the deferred provider-track item, NOT touched here.
- **No-leak / boundaries:** no raw prompt or completion is persisted or printed; the API key never enters the prefill path or any result; no provider metadata, billing, apply, preview, or schema strictness touched.

**Tests:** [`anthropicClient.test.ts`](../../../tests/unit/services/ai/modelClients/anthropicClient.test.ts) (request now carries the `{` prefill turn; +4 — re-attach to a mid-object continuation, no double-`{` on a full object, blank continuation → EMPTY_RESPONSE, prefill skipped when ending on an assistant turn), [`buildWorkflowPlanPrompt.test.ts`](../../../tests/unit/services/ai/planner/buildWorkflowPlanPrompt.test.ts) (+5 — JSON-only rules present, exactly-one-object, no-fences, first-`{`/last-`}`, no prose/comments/trailing commas), [`parseWorkflowPlanResponse.test.ts`](../../../tests/unit/services/ai/planner/parseWorkflowPlanResponse.test.ts) (+4 — preamble-before, trailing-prose-after, `//` comment, trailing comma all → NOT_JSON), [`BuilderAiPanel.test.tsx`](../../../tests/unit/features/workflow-builder/panels/BuilderAiPanel.test.tsx) (+1 — NOT_JSON-specific copy + value-free detail + no raw-message leak). No live model/network calls — the adapter fetch is injected.
