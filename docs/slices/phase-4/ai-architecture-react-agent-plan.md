...
**Interactive control ([`features/workflow-builder/ai/RequiredInputControl.tsx`](../../../features/workflow-builder/ai/RequiredInputControl.tsx)).** Per missing field, the component branches:

- `options[]` (static enum) → native `<select>` with one `<option>` per enum value.
...
- Neither → free-text `<input>` fallback.
