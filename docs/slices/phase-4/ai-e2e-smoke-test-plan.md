...
| Panel — load | [`BuilderAiPanel.persistedThread.test.tsx`](../../../tests/unit/features/workflow-builder/panels/BuilderAiPanel.persistedThread.test.tsx) `describe("AI-23 — persisted thread load")` (2 cases) | Mount with persisted messages → rehydrates user + assistant bubbles; persisted plan_result renders as `builder-ai-plan-result-previous` (read-only summary) with NO `builder-ai-apply-button`; thread-load failure falls back to the empty-chat intro hint (no blocking error). |
