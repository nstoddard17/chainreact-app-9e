...
**Companions:** [`task-cost-billing-model-audit.md`](./task-cost-billing-model-audit.md) (COST-1 audit) · [`task-cost-billing-foundation-closeout.md`](./account-settings/task-cost-billing-foundation-closeout.md) (COST-8 closeout)
...
| **COST-15C** | shipped | Engine wired to **create-at-start + finalize-update in flat mode**: `createWorkflowRunStart` (status 'running') before billing, `finalizeRun` UPDATE at finalize (recordRun INSERT fallback), `markWorkflowRunFailedBeforeExecution` for BILLING_EXHAUSTED, duplicate-dispatch guard. **Flat billing authoritative; no reserve/reconcile RPC called; shadow/usage unchanged.** Running rows filtered from user-facing reads (display types/UI untouched). See [pre-run-workflow-run-lifecycle-design.md](./pre-run-workflow-run-lifecycle-design.md) COST-15C note. |
| ~~COST-15D~~ | **superseded by COST-15H** | Was "internal-user allowlist" rollout. **Allowlist dropped** — app is pre-launch with no external users (decision 2026-05-25), so the global flag is the only switch. Engine integration moved to COST-15H. |
| **COST-15F** | shipped | Stale-`running`-run sweep (crash recovery): `sweepStaleRunningWorkflowRuns` repo method + `staleWorkflowRunSweep` service + guarded `sweep:stale-runs` ops script. Marks runs left 'running' past a cutoff (default 60 min, by `started_at`) failed with `EXECUTION_INTERRUPTED`. **Billing fields preserved; no balance mutation; separate from `release_expired_reservations`.** Idempotent; dev-DB verified. Cron wiring is a future deployment decision. See [pre-run-workflow-run-lifecycle-design.md](./pre-run-workflow-run-lifecycle-design.md) COST-15F note. |
| **COST-15G** | shipped | Internal rollout readiness plan ([reserve-reconcile-internal-rollout-readiness.md](./account-settings/reserve-reconcile-internal-rollout-readiness.md)). Doc only. **Allowlist section since rejected** (pre-launch); the rest (shadow thresholds, ops/sweep, rollback, risks) stands. |
| **COST-15H** | shipped | **Engine live reserve/reconcile behind the global flag** (no allowlist — pre-launch). `ENABLE_RESERVE_RECONCILE_BILLING=true` (real run) → reserve(estimate) before side effects → execute → reconcile(actual) (reconcile-before-finalize) → refund unused; insufficient reserve → BILLING_EXHAUSTED before any handler. Flag off → flat gate (unchanged rollback path). Test mode → no billing in either mode. **First balance-affecting engine integration.** See [pre-run-workflow-run-lifecycle-design.md](./pre-run-workflow-run-lifecycle-design.md) COST-15H note. |
| **COST-15I** | shipped | **Dev-DB live reserve/reconcile ENGINE verification, flag ON.** Gated integration harness ([`tests/integration/billing/reserveReconcileEngine.dev.test.ts`](../../../tests/integration/billing/reserveReconcileEngine.dev.test.ts)) drives the REAL `WorkflowEngine.runWorkflow` against the dev DB (ref `qcepijemjlkssfkvzlio`) with `ENABLE_RESERVE_RECONCILE_BILLING=true` (in-process only). **9 cases green (10/10 incl. sanity)**, real balance mutation verified end-to-end: pre-run row → reserve RPC → mocked handlers → reconcile RPC → `user_billing`/`workflow_runs`/`task_usage_events`. **No bug found, no engine change.** See note below. |
| **COST-15K** | shipped | **Scheduled the two cleanup sweeps** (ops/cron wiring only — no billing-logic change). Two protected cron routes ([`/api/cron/sweep-stale-runs`](../../../app/api/cron/sweep-stale-runs/route.ts) + [`/api/cron/release-expired-reservations`](../../../app/api/cron/release-expired-reservations/route.ts)), both `requireCronAuth` Bearer-`CRON_SECRET` protected, wired in [`vercel.json`](../../../vercel.json) at `*/10 * * * *`. Stale-run sweep delegates to COST-15F `sweepStaleRunningWorkflowRuns` (60-min cutoff, batched, lifecycle-only); reservation sweep delegates to COST-13 `releaseExpiredBillingReservations` (the non-flag-gated janitor → COST-12 RPC, service_role). Closes the crash-recovery safety gap. See note below. |
...
### COST-15I implementation note

**Dev-DB live reserve/reconcile ENGINE verification — the COST-15H engine path was exercised end-to-end against the real dev DB with the LIVE flag ON, proving real balance mutation. No bug found; no engine/RPC change.** COST-15H's unit tests mock the service layer; COST-15I closes that gap by running the real engine → real service → real RPCs → real `user_billing`/`workflow_runs`/`task_usage_events`.

- **Flag scope:** `ENABLE_RESERVE_RECONCILE_BILLING="true"` set **only in-process for the harness run** (`beforeAll`, restored in `afterAll`) against the confirmed disposable dev DB (ref `qcepijemjlkssfkvzlio` — the same DB authorized in COST-12C/14D/14E). `ENABLE_RESERVE_RECONCILE_SHADOW` stays off except inside case 9. No `.env` committed; no production access.
- **Harness** [`tests/integration/billing/reserveReconcileEngine.dev.test.ts`](../../../tests/integration/billing/reserveReconcileEngine.dev.test.ts) (opt-in: `ALLOW_DB_INTEGRATION_TESTS=true` + service-role env; self-skips otherwise — safe under plain `npm test`/CI; modeled on the COST-14E shadow harness). Drives the **REAL** engine; only the *outside world* is stubbed — action-handler registry (controllable mock handler, **no provider API ever called**), discovery registry (deterministic billable/trigger meta; real `taskCostPolicy` classifies), `notifyWorkflowFailure` (no-op), cookie client (throws if touched). `estimateWorkflowTaskCost`, `createBillingReservation`/`reconcileBillingReservation` → repo wrappers → COST-12 RPCs, the pre-run row lifecycle, and `computeRunTaskUsage`/`recordRunActuals` are all real.
  - Run: `ALLOW_DB_INTEGRATION_TESTS=true npx jest tests/integration/billing/reserveReconcileEngine.dev.test.ts` (PowerShell: `$env:ALLOW_DB_INTEGRATION_TESTS="true"; npx jest tests/integration/billing/reserveReconcileEngine.dev.test.ts`).
- **Cases executed (10/10 green, fresh throwaway user per case):**
  1. **One-action** — reserve 1 → reconcile 1; `tasks_used 10→11`, `tasks_reserved 0`, run `reconciled`, reserved/reconciled cost `1/1`, `billing_reconciled_at` set, 1 `node_task_charged`. **No flat deduction** (used moved by exactly the reconcile charge).
  2. **Three-action** — reserve 3 → reconcile 3; `tasks_used 0→3`, reserved 0, 3 node charges.
  3. **Branching (estimate > actual)** — router picks branch "a"; reserve 2 → reconcile 1, **refund 1**; `tasks_used 0→1`, reserved 0, reserved/reconciled `2/1`.
  4. **Trigger-only / zero-billable** — reserve 0 (`zero_reservation`) → reconcile 0; `tasks_used` unchanged, reserved 0, reserved/reconciled `0/0`, 0 node charges.
  5. **Partial failure** — a1 succeeds, a2 throws, a3 never runs; reserve 3 → reconcile **1** (succeeded billable only); run **failed**, `billing_status='reconciled'`, `tasks_used 0→1`, reserved 0.
  6. **Insufficient tasks** — limit 5/used 4, estimate 2; reserve **fails before any handler**; `BILLING_EXHAUSTED`; `tasks_used`/`tasks_reserved` unchanged (4/0); run `failed`, `billing_status='failed'`, `reconciled_task_cost` NULL, **0 ledger rows**.
  7. **Test mode** — no reserve, no reconcile, no flat; `tasks_used` unchanged, reserved 0; `billing_status` NULL, reserved/reconciled NULL; **0 ledger rows, 0 shadow rows**.
  8. **Rollback (flag OFF)** — flat `deduct_tasks_if_available` deducts **1/run** regardless of 3 actions; `tasks_used 0→1`, reserved 0; `billing_status` NULL (no reserve/reconcile); COST-3 ledger still records 3 node charges.
  9. **Shadow ON + reserve mode** — reconcile is still the **only** balance mutation (`tasks_used 0→2`); one `billing_shadow_comparisons` row persists (read-only side-channel, no extra mutation).
- **Balance-mutation findings:** every reserve/reconcile case mutated `user_billing` exactly as designed — `tasks_reserved` returned to **0** in all cases (hold fully settled or released), `tasks_used` moved by **exactly the reconciled charge** (proving flat `deduct` is never called in reserve mode), and refunds matched estimate−actual (case 3: 1, case 5: 2). `workflow_runs` carried the right `billing_status` / `reserved_task_cost` / `reconciled_task_cost` / `billing_reconciled_at`; `task_usage_events` carried one `run_estimate_recorded` + one `node_task_charged` per successful billable node. Engine logs confirmed the live ordering: `createWorkflowRunStart` → `reserveTasks` RPC → `billing_reserved` → steps → `reconcileReservation` RPC → `billing_reconciled` → `finalizeWorkflowRun` → `task_usage_events insert`.
- **Cleanup:** explicit FK-safe deletes by seeded user ids (shadow/usage/runs/workflows/billing) + `auth.admin.deleteUser` cascade. Post-run residue probe found **0** leftover `cost15i*` workflows and **0** `@chainreact-rr-engine.invalid` users.
- **Evaluation (next-gate):** the live engine reserve/reconcile path is **functionally correct against a real DB** — the balance math, the pre-run-row → reserve → reconcile lifecycle, the partial/insufficient/zero/test/rollback branches, and ledger writes all behave as designed. This is **synthetic harness data** (mock handlers, one run per shape, single-process — no concurrency), so it proves *correctness of the engine path*, not real-world distribution or concurrent-run behavior under load. Reserve/reconcile can be considered for the pre-launch default **after** (a) representative organic dev/internal usage is observed with the flag on, and (b) the sweeps are scheduled (COST-15K: `release_expired_reservations` + stale-`running` sweep) so an engine crash between reserve and reconcile cannot strand a hold in production. Do **not** read this as "ready for production cutover" (that is COST-16).
- **Unchanged:** flat gate still authoritative when the flag is off (rollback proven in case 8); `deduct_tasks_if_available` not removed; no UI, no owner route, no AI, no templates, no custom nodes.

### COST-15K implementation note

**Scheduled the two cleanup sweeps reserve/reconcile needs before it can safely become the pre-launch default. Ops/cron wiring ONLY — no reserve/reconcile billing-logic change, flat rollback intact.** Both sweeps were already implemented and manually runnable (COST-15F service/script; COST-12 RPC + COST-13 service); the gap was that an engine crash after creating a `running` row or after reserving tasks needed *scheduled* cleanup, not only manual.

- **Scheduling pattern (audit):** the project schedules via **Vercel cron** ([`vercel.json`](../../../vercel.json) `crons`) → `app/api/cron/<name>/route.ts` (GET for Vercel + POST for manual) → `requireCronAuth(request)` ([`services/cron/auth.ts`](../../../services/cron/auth.ts), Bearer `CRON_SECRET`, `timingSafeEqual`) → service delegate → counts-only JSON, fail-safe 500 + structured `console.error`. Existing crons: `poll-triggers` (every min), `renew-watch-subscriptions` (every 10 min); routes `cleanup-workflow-files` + `run-scheduled-triggers` exist but weren't in `vercel.json`. COST-15K follows this pattern exactly.
- **Stale-running sweep** — route [`app/api/cron/sweep-stale-runs/route.ts`](../../../app/api/cron/sweep-stale-runs/route.ts) → COST-15F [`sweepStaleRunningWorkflowRuns`](../../../services/execution/staleWorkflowRunSweep.ts). Cutoff 60 min (service default `STALE_RUNNING_RUN_DEFAULT_AGE_MS`), batched (route `DEFAULT_BATCH_LIMIT=500`, overridable via `?limit=<n>`). Summary log `cron.sweep_stale_runs.done` with `sweptCount` / `cutoff` / `olderThanMs` / `limit`; response `{ ok, sweptCount, cutoff, olderThanMs }` (no run ids). **Lifecycle cleanup only — no balance mutation, no reserve/reconcile RPC, no `task_usage_events`.**
- **Expired-reservation sweep** — route [`app/api/cron/release-expired-reservations/route.ts`](../../../app/api/cron/release-expired-reservations/route.ts) → COST-13 [`releaseExpiredBillingReservations`](../../../services/billing/reserveReconcileBilling.ts) → COST-12 `release_expired_reservations` RPC (service_role). **Deliberately NOT flag-gated** (the janitor is safe anytime; a `ENABLE_RESERVE_RECONCILE_BILLING` rollback must never strand reserved tasks). Summary log `cron.release_expired_reservations.done` with `releasedCount` / `releasedTasks`; response `{ ok, releasedCount, releasedTasks }`. Service catches RPC errors and returns `ok:false` (never throws) → route maps to 500.
- **Cadence:** both wired in `vercel.json` at `*/10 * * * *` (every 10 min), matching `renew-watch-subscriptions`.
- **Auth/secret:** reuses the existing `requireCronAuth` convention — Bearer `CRON_SECRET`; missing/empty `CRON_SECRET` → 500 (misconfig, not silent allow); missing/wrong Bearer → 401. No secret printed or committed; no `.env` change.
- **Tests** ([`tests/unit/app/api/cron/sweep-stale-runs.route.test.ts`](../../../tests/unit/app/api/cron/sweep-stale-runs.route.test.ts) + [`release-expired-reservations.route.test.ts`](../../../tests/unit/app/api/cron/release-expired-reservations.route.test.ts)): use the **real** `requireCronAuth` with a test `CRON_SECRET` so secret behavior is proven end-to-end — **missing secret → 401, wrong secret → 401, correct secret → 200, unset `CRON_SECRET` → 500** — plus service delegation, `?limit=` forwarding + default fallback, GET+POST, service-throws → generic 500, `ok:false` → 500, and counts-only responses. The COST-15F sweep service, COST-13 `releaseExpiredBillingReservations`, and the reserve/reconcile engine tests all still pass unchanged.
- **Live billing unchanged:** no change to `executionBillingGate` / `deduct_tasks_if_available` / reserve/reconcile services or RPCs; flat rollback still authoritative when the flag is off; no UI, no owner dashboard, no AI, no templates, no custom nodes.
- **Readiness:** with both sweeps scheduled, the reserve/reconcile **ops-safety gap is closed** — a crash between reserve and reconcile is now reclaimed automatically (hold released by the reservation sweep; the stuck `running` row finalized by the stale-run sweep). Reserve/reconcile can now be **considered for the pre-launch default** after enough organic dev/internal confidence with the flag on (production cutover remains COST-16; flat-gate removal remains COST-17). **The next non-billing priority is provider metadata/builder readiness for the 9 pending launch-scope providers — NOT resuming AI implementation.**
