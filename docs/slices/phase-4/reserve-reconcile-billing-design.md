...
| COST-15 | future (sample shadow data collected; needs MORE representative data + pre-run row) | Internal users (live reserve/reconcile for an allowlist) — gate on aggregated shadow data; requires creating the `workflow_runs` row at reserve time. |
