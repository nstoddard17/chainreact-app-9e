...
- **Signup verification is a 6-digit code, not a link (Slice AUTH-EMAIL-OTP-1).** `signUp` still creates the unconfirmed user; `features/auth/SignUpFlow.tsx` then swaps the form for `VerifyEmailForm`, which redeems the code through the `verifySignupOtp` server action (`supabase.auth.verifyOtp({ type: "signup" })`) and resends through `resendSignupOtp` (`supabase.auth.resend({ type: "signup" })`, Turnstile-forwarded). **This depends on the Supabase signup email template rendering `{{ .Token }}`** — templates are Dashboard-managed, not in this repo. `/auth/callback` is still required and unchanged: Google OAuth and password recovery both go through it. The pending address lives in React state only — never storage, cookie, or URL — so a hard refresh returns to the signup form by design; the password is never captured past the original submit.
