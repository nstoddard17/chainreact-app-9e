...
**Unblocks:** AIRTABLE-META-2 (`airtable:fields` / `:views` / `:attachment_fields` resolvers — see [`airtable-metadata-coverage-plan.md`](../providers/airtable-metadata-coverage-plan.md)), and any future provider/custom node whose option resolver depends on more than one upstream field.
