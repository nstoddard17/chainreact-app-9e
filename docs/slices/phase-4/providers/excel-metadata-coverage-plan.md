...
> **✅ EXCEL-META-3 shipped (2026-05-25).** All 10 ActionMeta + 5 polling TriggerMeta added; `services/discovery/providers/microsoft-excel.ts` sub-registry wired into the discovery registry; **`microsoft-excel` flipped into `COVERED_PROVIDERS`** — Excel is now builder-visible (`hasMetadata:true`, no longer "coming soon"). `delete_row` + `delete_worksheet` are high/destructive/requiresConfirmation (Marcus decision). `columns` resolver remains deferred. Covered providers 18 → **19/26**; pending 8 → **7**.
