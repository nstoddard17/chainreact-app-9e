...
- **Stale 'running' rows (COST-15F, future):** a crash between create and finalize leaves a `running` row. It is hidden from the UI by the filter above; a sweep to finalize stale running rows (companion to the COST-12 `release_expired_reservations` hold sweep) remains a documented follow-up. Normal failures finalize the row, so controlled paths never produce stale rows (covered by the failed-run tests).
