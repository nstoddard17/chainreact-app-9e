...
import { InstagramAdapter } from '../providers/instagram-adapter'
...
import { BoxAdapter } from '../providers/box-adapter'
import { MailchimpAdapter } from '../providers/mailchimp-adapter'
import { ManyChatAdapter } from '../providers/manychat-adapter'
...
export function bootstrapProviders(): void {
  ...
  // Register ManyChat provider
  registerManyChatProvider()
...
function registerInstagramProvider(): void {
  ...
  // Register Instagram actions
  actionRegistry.registerProvider('instagram', [
    {
      actionType: 'create_post',
      handler: async (config, context) => {
        const provider = providerRegistry.getSocialProvider('instagram')
        if (!provider) throw new Error('Instagram provider not available')
        
        return provider.createPost({
          content: config.parameters.content || config.parameters.caption || config.parameters.text,
          mediaFiles: config.parameters.mediaFiles || config.parameters.images || config.parameters.media,
          hashtags: config.parameters.hashtags,
          mentions: config.parameters.mentions,
          scheduledTime: config.parameters.scheduledTime ? new Date(config.parameters.scheduledTime) : undefined,
          metadata: {
            location: config.parameters.location,
            altText: config.parameters.altText
          }
        }, context.userId)
      },
      metadata: {
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        await provider.deletePost(config.parameters.postId, context.userId)
        
        return {
          success: true,
          output: { postId: config.parameters.postId },
          message: 'Instagram post deleted successfully'
        }
      },
      metadata: {
        name: 'Delete Post',
        description: 'Delete an Instagram post',
        version: '1.0.0',
        category: 'social'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.commentOnPost(
          config.parameters.postId,
          config.parameters.comment || config.parameters.text || config.parameters.message,
          context.userId
        )
      },
      metadata: {
    ...
    },
    {
      ...
      metadata: {
        name: 'Get Mentions',
        description: 'Get mentions and comments from Instagram',
        version: '1.0.0',
        category: 'social'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.getInsights({
          metric: config.parameters.metric || 'impressions',
          period: config.parameters.period || 'day',
          periodCount: config.parameters.periodCount || 1,
          startDate: config.parameters.startDate ? new Date(config.parameters.startDate) : undefined,
          endDate: config.parameters.endDate ? new Date(config.parameters.endDate) : undefined
        }, context.userId)
      },
      metadata: {
        name: 'Get Insights',
        description: 'Get Instagram account and post insights/analytics',
        version: '1.0.0',
        category: 'social'
      }
    }
  ...
  console.log('✅ Instagram provider registered with social capabilities')
}

function registerLinkedInProvider(): void {
  ...
  actionRegistry.registerProvider('linkedin', [
    {
      actionType: 'create_post',
      handler: async (config, context) => {
        const provider = providerRegistry.getSocialProvider('linkedin')
        if (!provider) throw new Error('LinkedIn provider not available')
        
        return provider.createPost({
          content: config.parameters.content || config.parameters.text || config.parameters.message,
          mediaFiles: config.parameters.mediaFiles || config.parameters.links || config.parameters.urls,
          hashtags: config.parameters.hashtags,
          mentions: config.parameters.mentions,
          scheduledTime: config.parameters.scheduledTime ? new Date(config.parameters.scheduledTime) : undefined,
          metadata: {
            visibility: config.parameters.visibility || 'PUBLIC',
            companyPage: config.parameters.companyPage
          }
        }, context.userId)
      },
      metadata: {
        name: 'Create Post',
        description: 'Create a new LinkedIn post with text and optional media',
        version: '1.0.0',
        category: 'social'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.likePost(config.parameters.postId, context.userId)
      },
      metadata: {
        name: 'Like Post',
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.sendDirectMessage({
          recipientId: config.parameters.recipientId || config.parameters.userId || config.parameters.personId,
          content: config.parameters.content || config.parameters.message || config.parameters.text,
          mediaFiles: config.parameters.mediaFiles,
          metadata: {
            conversationId: config.parameters.conversationId
          }
        }, context.userId)
      },
      metadata: {
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.getInsights({
          metric: config.parameters.metric || 'profile_views',
          period: config.parameters.period || 'day',
          periodCount: config.parameters.periodCount || 1,
          startDate: config.parameters.startDate ? new Date(config.parameters.startDate) : undefined,
          endDate: config.parameters.endDate ? new Date(config.parameters.endDate) : undefined
      ...
      metadata: {
        name: 'Get Insights',
        description: 'Get LinkedIn profile and post insights/analytics',
        version: '1.0.0',
        category: 'social'
      }
    }
  ...
  console.log('✅ LinkedIn provider registered with social capabilities')
}
...
function registerShopifyProvider(): void {
  const shopifyAdapter = new ShopifyAdapter()
  
  // Register provider with capabilities
  providerRegistry.register(
    shopifyAdapter,
    ['ecommerce'], // capability types
    { name: 'Shopify', version: '1.0.0' }
  )

  // Register Shopify actions
  actionRegistry.registerProvider('shopify', [
    {
      actionType: 'create_product',
      handler: async (config, context) => {
        const provider = providerRegistry.getEcommerceProvider('shopify')
        if (!provider) throw new Error('Shopify provider not available')
        
        return provider.createProduct({
          title: config.parameters.title || config.parameters.name,
          description: config.parameters.description || config.parameters.body_html,
          price: config.parameters.price,
          compareAtPrice: config.parameters.compareAtPrice || config.parameters.compare_at_price,
          currency: config.parameters.currency,
          images: config.parameters.images,
          variants: config.parameters.variants,
          tags: config.parameters.tags,
          vendor: config.parameters.vendor,
          productType: config.parameters.productType || config.parameters.product_type,
          published: config.parameters.published,
          metadata: config.parameters.metadata
        }, context.userId)
      },
      metadata: {
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.updateProduct(
          config.parameters.productId,
          {
            title: config.parameters.title,
            description: config.parameters.description,
            price: config.parameters.price,
            compareAtPrice: config.parameters.compareAtPrice,
            images: config.parameters.images,
            variants: config.parameters.variants,
            tags: config.parameters.tags,
            vendor: config.parameters.vendor,
            productType: config.parameters.productType,
            published: config.parameters.published,
            metadata: config.parameters.metadata
          },
          context.userId
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.getProduct(config.parameters.productId, context.userId)
      },
      metadata: {
        name: 'Get Product',
        description: 'Get product details from Shopify',
        version: '1.0.0',
        category: 'ecommerce'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.getProducts({
          ...
          published: config.parameters.published,
          tags: config.parameters.tags,
          priceRange: config.parameters.priceRange,
          limit: config.parameters.limit || 50
        }, context.userId)
      },
      metadata: {
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.createOrder({
          customerId: config.parameters.customerId,
          lineItems: config.parameters.lineItems || config.parameters.line_items,
          shippingAddress: config.parameters.shippingAddress || config.parameters.shipping_address,
          billingAddress: config.parameters.billingAddress || config.parameters.billing_address,
          email: config.parameters.email,
          phone: config.parameters.phone,
          note: config.parameters.note,
          tags: config.parameters.tags,
          financialStatus: config.parameters.financialStatus || config.parameters.financial_status,
          fulfillmentStatus: config.parameters.fulfillmentStatus || config.parameters.fulfillment_status,
          metadata: config.parameters.metadata
        }, context.userId)
      },
      metadata: {
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.updateOrder(
          config.parameters.orderId,
          {
            email: config.parameters.email,
            phone: config.parameters.phone,
            note: config.parameters.note,
            tags: config.parameters.tags,
            financialStatus: config.parameters.financialStatus,
            fulfillmentStatus: config.parameters.fulfillmentStatus,
            metadata: config.parameters.metadata
          },
          context.userId
        )
    ...
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.getOrder(config.parameters.orderId, context.userId)
      },
      metadata: {
        name: 'Get Order',
    ...
    },
    {
      ...
      metadata: {
        name: 'Get Orders',
        description: 'Get list of orders from Shopify',
        version: '1.0.0',
        category: 'ecommerce'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.updateInventory(
          config.parameters.productId,
          config.parameters.quantity || config.parameters.inventory_quantity,
          context.userId
        )
      },
      metadata: {
    ...
    {
      actionType: 'get_inventory',
      handler: async (config, context) => {
        const provider = providerRegistry.getEcommerceProvider('shopify')
        if (!provider) throw new Error('Shopify provider not available')
        
        return provider.getInventory({
          productId: config.parameters.productId,
          sku: config.parameters.sku,
          lowStock: config.parameters.lowStock,
          limit: config.parameters.limit || 100
        }, context.userId)
      },
      metadata: {
  ...
  console.log('✅ Shopify provider registered with ecommerce capabilities')
}
...
function registerBoxProvider(): void {
  const boxAdapter = new BoxAdapter()
  ...
  // Register Box actions
  actionRegistry.registerProvider('box', [
    {
      actionType: 'upload_file',
      handler: async (config, context) => {
        const provider = providerRegistry.getFileProvider('box')
        if (!provider) throw new Error('Box provider not available')
  ...
  console.log('✅ Box provider registered with file capabilities')
}
...
function registerManyChatProvider(): void {
  const manyChatAdapter = new ManyChatAdapter()
  
  // Register provider with capabilities
  providerRegistry.register(
    manyChatAdapter,
    ['chat'], // capability types
    { name: 'ManyChat', version: '1.0.0' }
  )

  // Register ManyChat actions
  actionRegistry.registerProvider('manychat', [
    {
      actionType: 'send_message',
      handler: async (config, context) => {
        const provider = providerRegistry.getChatProvider('manychat')
        if (!provider) throw new Error('ManyChat provider not available')
        
        return provider.sendMessage({
          channelId: config.parameters.subscriberId || config.parameters.channelId,
          content: config.parameters.message || config.parameters.content || config.parameters.text,
          attachments: config.parameters.attachments,
          metadata: {
            type: config.parameters.type || 'text'
          }
        }, context.userId)
      },
      metadata: {
        name: 'Send Message',
        description: 'Send a message to a subscriber via ManyChat',
        version: '1.0.0',
        category: 'chat'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.createChannel({
          name: config.parameters.name || config.parameters.tagName,
          description: config.parameters.description,
          private: false
        }, context.userId)
      },
      metadata: {
        name: 'Create Tag',
        description: 'Create a tag in ManyChat',
        version: '1.0.0',
        category: 'chat'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.manageMembers({
          type: 'add',
          channelId: config.parameters.tagId?.toString() || config.parameters.channelId,
          memberIds: Array.isArray(config.parameters.subscriberIds) 
            ? config.parameters.subscriberIds 
            : [config.parameters.subscriberId]
        }, context.userId)
      },
      metadata: {
        name: 'Add Tag',
        description: 'Add a tag to subscribers in ManyChat',
        version: '1.0.0',
        category: 'chat'
      }
    },
    {
      ...
      metadata: {
        name: 'Remove Tag',
        description: 'Remove a tag from subscribers in ManyChat',
        version: '1.0.0',
        category: 'chat'
      }
    },
    {
      ...
      metadata: {
        name: 'Get Tags',
        description: 'Get tags from ManyChat',
        version: '1.0.0',
        category: 'chat'
      }
    },
    {
      ...
      handler: async (config, context) => {
        ...
        return provider.getMembers(
          config.parameters.tagId?.toString() || config.parameters.channelId,
          context.userId
        )
      },
      metadata: {
        name: 'Get Subscribers',
        description: 'Get subscribers with a specific tag from ManyChat',
        version: '1.0.0',
        category: 'chat'
      }
    }
  ...
  console.log('✅ ManyChat provider registered with chat capabilities')
}
