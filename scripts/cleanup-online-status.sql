-- Cleanup script to remove online status tracking from database
-- Since we're using a simpler frontend-only approach

-- Remove the last_seen_at column from user_profiles
ALTER TABLE user_profiles DROP COLUMN IF EXISTS last_seen_at;

-- Drop all activity tracking triggers
