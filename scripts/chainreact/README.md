...
reflects current source. Exit codes: `0` ok, `1` validation/verification failure,
`2` usage error.
...
result shape + renderer already support it. Natural next checks: OAuth scope
declarations, field/schema definitions, AI metadata, builder visibility, icons,
categories, and deeper trigger/action validation. Keep the no-import / text-scan
posture (do not pull app code into the CLI) and keep findings actionable.
