...
export function renderProviderList(infos: readonly ProviderInfo[]): string {
  ...
  const header = `  ${"id".padEnd(idW)}  ${"displayName".padEnd(nameW)}  enabled  actions  meta  schema  trigMeta`;
  const lines: string[] = [`ChainReact — app list (${infos.length} provider(s))`, "", header];
  for (const i of infos) {
    lines.push(
      `  ${i.id.padEnd(idW)}  ${(i.displayName ?? "-").padEnd(nameW)}  ${enabledLabel(i.enabled).padEnd(7)}  ${String(i.counts.actionHandlers).padStart(7)}  ${String(i.counts.actionMetas).padStart(4)}  ${String(i.counts.actionSchemas).padStart(6)}  ${String(i.counts.triggerMetas).padStart(8)}`,
    );
  }
  return lines.join("\n");
}
