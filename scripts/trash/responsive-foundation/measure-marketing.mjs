/**
 * RESPONSIVE-MARKETING-9 — public marketing funnel continuous width+height verification.
 *
 * A sibling of `measure-auth.mjs`, with the SAME three assertion classes
 * (containment · legibility floors · horizontal-panning policy) per
 * `docs/rules/responsive-layout-and-validation.md`, plus the vertical-reachability
 * pass auth introduced.
 *
 * Separate from the shell harness for the same structural reason auth is: the
 * public pages render no authenticated app shell. They are full-bleed surfaces
 * that bring their own scoped `--mk-*` stylesheets inside the fragment, so
 * wrapping them in rail + top bar would measure a page that does not exist.
 *
 * The reachability probe targets the marketing primary CTA rather than a form
 * submit: on a marketing page the equivalent "can the visitor still act" question
 * is whether the signup call to action can be reached.
 *
 * Run:
...
const SHOT_DIR = join(process.cwd(), "owner-review", "responsive-marketing");
...
/** Every bounded region of the public marketing surface, checked individually. */
const REGIONS = [
  // Shared chrome
  ".marketing-root",
  ".mk-nav",
...
const fragments = readdirSync(HTML_DIR)
  .filter((f) => /^mk-/.test(f) && f.endsWith(".html"))
  .sort();
if (fragments.length === 0) {
  console.error("No mk-*.html fragments. Run the harness test first.");
  process.exit(2);
}
...
async function measure(width, height) {
  return page.evaluate((regionSelectors) => {
    ...
    for (const sel of regionSelectors) {
      for (const el of document.querySelectorAll(sel)) {
        ...
        for (const child of el.children) {
          ...
          // RESPONSIVE-MARKETING-9 — the DECLARED-SCROLLER exemption the deep
          // descendant pass already applies, applied here too.
          //
          // The deep pass skips a node whose nearest box-generating ancestor
          // declares `overflow-x: auto|scroll`, because content wider than an
          // opt-in scroller is the POINT. This direct-child pass never learned
          // that rule — auth had no legitimate scroller, so the gap was inert.
          // The public pricing matrix is the first one: a 720px comparison table
          // inside `.pr-cmp-scroll` was reported as "escaping" its own scroller
          // at 68 sizes, which says nothing about whether the visitor is harmed.
          //
          // Coverage moves to `data-no-pan-below`, which the comparison SECTION
          // now declares: the matrix may pan inside its card, the section around
          // it may not.
          //
          // HONEST LIMIT, measured — do not overstate this. Reverting the
          // scroller's `max-width` bound produces ZERO failures in any assertion
          // class, because a block-level scroller is already sized by its
          // containing block; the bound is DEFENSIVE, not load-bearing. And the
          // suppressed "escape" was a false positive throughout: the visitor was
          // never harmed by it. So this exemption removed noise, and no real
          // defect moved out from under an assertion with it. The behaviour that
          // geometry genuinely cannot see here — is every feature row still
          // present at narrow widths — is covered by rendered tests instead, in
          // tests/unit/features/marketing/marketingResponsive.test.tsx.
          // See docs/rules/responsive-layout-and-validation.md §A/§C.
          if (isDeclaredScroller) continue;
    ...
    const submit = document.querySelector(".mk-btn-primary, .pr-cta, .mk-hero-cta a");
    if (submit) {
      ...
      if (absTop < -1) {
        out.unreachable.push({ what: "primary CTA", reason: `clipped ${Math.round(-absTop)}px above the document origin` });
      }
...
if (unique.length === 0) {
  console.log("\nPASS — no horizontal overflow, no region escapes, no illegible or pannable region, primary CTA reachable at every measured size.");
  process.exit(0);
}
