...
- React Agent capability truth + turn integrity (registered node metadata decides buildability; polling triggers ARE triggers; the model can't override deterministic capability facts; a proposal renders in the turn that produced it; stale responses mutate nothing): [`docs/rules/react-agent-capability-truth-and-turn-integrity.md`](./docs/rules/react-agent-capability-truth-and-turn-integrity.md)
- Workflow revision concurrency (every authoritative definition save is an atomic compare-and-swap on `updated_at` with a required `expectedRevision`; stale saves get a typed 409 + the shared changed-elsewhere recovery; silent overwrite is forbidden): [`docs/rules/workflow-revision-concurrency.md`](./docs/rules/workflow-revision-concurrency.md)
