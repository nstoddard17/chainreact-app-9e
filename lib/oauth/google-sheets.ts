...
export class GoogleSheetsOAuthService {
  ...
  static getRedirectUri(): string {
    return getOAuthRedirectUri("google-sheets")
  }
  ...
  static generateAuthUrl(baseUrl: string, reconnect = false, integrationId?: string, userId?: string): string {
    ...
    const state = userId ? generateOAuthState("google-sheets", userId, { reconnect, integrationId }) : ""
  ...
  static async exchangeCodeForTokens(code: string): Promise<any> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Google Sheets token exchange failed: ${error}`)
    }
  ...
  static async getUserInfo(accessToken: string): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to get Google Sheets user info: ${response.statusText}`)
    }
