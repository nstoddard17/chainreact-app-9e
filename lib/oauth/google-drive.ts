...
export class GoogleDriveOAuthService {
  ...
  static getRedirectUri(): string {
    return getOAuthRedirectUri("google-drive")
  }
  ...
  static generateAuthUrl(baseUrl: string, reconnect = false, integrationId?: string, userId?: string): string {
    ...
    const state = userId ? generateOAuthState("google-drive", userId, { reconnect, integrationId }) : ""
  ...
  static async getUserInfo(accessToken: string): Promise<any> {
    ...
    if (!response.ok) {
      throw new Error(`Failed to get Google Drive user info: ${response.statusText}`)
    }
