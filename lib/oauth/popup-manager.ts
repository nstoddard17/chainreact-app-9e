...
export class OAuthPopupManager {
  ...
  static setupPopupListeners(
    ...
    const storageCheckPrefixes = [
      `oauth_response_${provider}`,
      provider === 'microsoft-onenote' ? 'oauth_response_microsoft-onenote' : null,
      provider === 'onenote' ? 'oauth_response_onenote' : null
    ].filter(Boolean)
    
    try {
      const allKeys = Object.keys(localStorage)
      storageCheckPrefixes.forEach(prefix => {
        const staleKeys = allKeys.filter(key => key.startsWith(prefix))
        staleKeys.forEach(key => {
          // Silently remove stale entries
        ...
        if (popup && popup.closed && !closedByMessage && !popupClosedManually) {
          ...
          if (!cancelCheckScheduled) {
            ...
            if (provider === 'hubspot') {
              ...
              // Microsoft providers need more time for the success message to be stored
              const isMicrosoftProvider = ['microsoft-onenote', 'onenote', 'microsoft-outlook', 'outlook', 'teams', 'onedrive'].includes(provider)
              const cancelDelay = isMicrosoftProvider ? 3500 : 2000
    ...
    // Check instantly for HubSpot and Microsoft providers
    const isMicrosoftProvider = ['microsoft-onenote', 'onenote', 'microsoft-outlook', 'outlook', 'teams', 'onedrive'].includes(provider)
    const storageCheckInterval = (provider === 'hubspot' || isMicrosoftProvider) ? 50 : 500
    const storageCheckTimer = setInterval(() => {
      // Continue checking even if popup closed manually - success might still be detected
      
      try {
        const allKeys = Object.keys(localStorage)
        // Check all possible prefixes for this provider
        const matchingKeys = allKeys.filter(key => {
          return storageCheckPrefixes.some(prefix => key.startsWith(prefix))
        })
  ...
  static async openReconnectionPopup(
    ...
    for (const key of allKeys) {
      if (storageCheckPrefixes.some(prefix => key.startsWith(prefix))) {
        localStorage.removeItem(key)
      }
    }
