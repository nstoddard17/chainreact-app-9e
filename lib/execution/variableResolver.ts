...
export function replaceTemplateVariables(template: string, data: any): any {
  ...
  logger.info(`🔧 Replacing variables in template length: ${template.length}`)
  logger.info(`🔧 Available data keys:`, Object.keys(data || {}))

  // Special debug for message content
  if (template.includes('Message Content')) {
    logger.info(`🔧 🚨 MESSAGE CONTENT DEBUG:`)
    logger.info(`🔧   - template length: ${template.length}`)
    logger.info(`🔧   - data.message exists: ${!!data?.message}`)
    logger.info(`🔧   - data.message.content length: ${data?.message?.content?.length || 0}`)
  }

  // Handle template syntax like {{New Message in Channel.Message Content}}
  return template.replace(/\{\{([^}]+)\}\}/g, (match, path) => {
    const trimmedPath = path.trim()
    logger.info(`🔧 Processing variable path: "${trimmedPath}"`)
    ...
    if (trimmedPath.includes('.')) {
      ...
      logger.info(`🔧 Node: "${nodeName}", Field: "${fieldName}"`)
...
function resolveDiscordMessageField(fieldName: string, data: any): any {
  switch (fieldName) {
    case 'Message Content':
      ...
      logger.info(`🔧 Found Message Content length: ${content.length}`)
      return content
    case 'Channel Name':
      const channelName = data?.message?.channelName || data?.message?.channelId || ''
      logger.info(`🔧 Found Channel Name length: ${channelName.length}`)
      return channelName
    case 'Author Name':
      const authorName = data?.message?.authorName || data?.message?.authorDisplayName || data?.message?.authorId || ''
      logger.info(`🔧 Found Author Name length: ${authorName.length}`)
      return authorName
    case 'Guild Name':
...
function resolveDiscordJoinField(fieldName: string, data: any): any {
  ...
  if (joinKey) {
    ...
    for (const candidate of candidatePaths) {
      ...
      if (resolved !== undefined && resolved !== null) {
        logger.info(`🔧 Found User Joined Server field "${fieldName}": "${resolved}"`)
        return resolved
      }
    }
  }

  logger.info(`🔧 User Joined Server output not found for field: "${fieldName}"`)
  return undefined
}
...
function resolveAIAgentField(fieldName: string, data: any): any {
  logger.info(`🔧 Looking for AI Agent output in data:`, Object.keys(data))
  ...
  logger.info(`🔧 AI Agent output not found for field: "${fieldName}"`)
  return undefined
}
