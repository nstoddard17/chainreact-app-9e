...
export async function sendSlackMessage(params: {
  ...
  try {
    ...
    // Get the Slack token using workspace (integration ID), integrationId from config, or userId as fallback
    // Priority: workspace field > integrationId from config > userId fallback
    const integrationIdToUse = workspace || integrationId

    // If sendAsUser is true, use the user token (xoxp-) instead of bot token (xoxb-)
    // This allows messages to appear as coming from the user instead of the bot
    const useUserToken = sendAsUser === true
    const tokenToUse = integrationIdToUse
      ? await getSlackToken(integrationIdToUse, true, useUserToken)
          ...
          if (typeof attachment === 'string') {
            ...
            if (attachment.filePath) {
              ...
              if (!error && storageFile) {
                ...
                const fileName = `slack-attachments/${userId}/${Date.now()}-${attachment.fileName || 'attachment'}`
              ...
              const fileData = await FileStorageService.getFile(attachment.id, userId)
              ...
              if (fileData) {
                ...
                const fileName = `slack-attachments/${userId}/${Date.now()}-${fileData.metadata.fileName}`
    ...
    return {
      ...
      output: {
        ...
        timestamp: result.ts,
        text: result.message?.text || slackTextContent,
