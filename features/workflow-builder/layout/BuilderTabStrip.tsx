...
/**
 * Header-level builder tab strip (BUILDER-TABS-HEADER-1).
 *
 ...
 * Document view) can reach Runs / Data Map / History / Settings. Rendered by
 * `WorkflowBuilder` directly under `BuilderHeader` in the shell's header
 * region; tab state lives in `WorkflowBuilder` (it now outlives view-mode
 * switches). Pure presentational — same roles/labels the canvas strip had,
 * so existing role-based queries keep working.
 */
...
export function BuilderTabStrip({
  ...
  return (
    <div
      ...
      className="flex h-9 shrink-0 items-center px-2.5"
      style={{
